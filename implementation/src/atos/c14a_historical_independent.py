"""Physically separate C14A signal, ledger, metric, and gate recomputation."""
...
class C14AHistoricalIndependentError(RuntimeError):
...
def _time(value: Any) -> datetime:
    try:
        ...
        raise C14AHistoricalIndependentError(f"invalid timestamp: {value!r}") from exc
    if result.tzinfo is None:
        raise C14AHistoricalIndependentError("timestamp must be timezone-aware")
    return result.astimezone(UTC)


def _number(value: Any, label: str, *, positive: bool = False) -> Decimal:
    if isinstance(value, bool):
        raise C14AHistoricalIndependentError(f"{label} must be decimal")
    try:
        result = Decimal(str(value))
    except (InvalidOperation, ValueError) as exc:
        raise C14AHistoricalIndependentError(f"{label} must be decimal") from exc
    if not result.is_finite() or (positive and result <= 0):
        raise C14AHistoricalIndependentError(f"{label} is non-finite or non-positive")
    return result
...
def _series(
    ...
    for row in rows:
        ...
        if stamp.minute or stamp.second or stamp.microsecond:
            raise C14AHistoricalIndependentError(f"{label} is off-grid")
        if last is not None and stamp <= last:
            raise C14AHistoricalIndependentError(f"{label} is duplicate or unordered")
        last = stamp
        output[stamp] = _number(row.get(field), f"{label} {field}", positive=True)
    if not output:
        raise C14AHistoricalIndependentError(f"{label} is empty")
    return output
...
def _require_funding_coverage(
    ...
    *,
    ...
    if not stamps:
        raise C14AHistoricalIndependentError(f"{label} funding coverage is empty")
    gaps = (
        stamps[0] - start,
    ...
    if any(gap < timedelta(0) or gap > maximum_gap for gap in gaps):
        raise C14AHistoricalIndependentError(f"{label} funding coverage drift")
...
def _signal(
    ...
    *,
    selected: tuple[str, ...],
    trades: Mapping[str, Mapping[datetime, Decimal]],
    quote_volumes: Mapping[str, Mapping[datetime, Decimal]],
    policy: str,
) -> dict[str, Any]:
    cutoff = stamp - SIGNAL_EXECUTION_LAG_HOURS * HOUR
    first = cutoff - SIGNAL_LOOKBACK_COMPLETE_DAYS * 24 * HOUR
    clock = [
        first + offset * 24 * HOUR
        for offset in range(SIGNAL_LOOKBACK_COMPLETE_DAYS + 1)
    ]
    returns: dict[str, list[Decimal]] = {}
    daily_quote_volumes: dict[str, list[Decimal]] = {}
    for instrument in selected:
        try:
            points = [trades[instrument][when] for when in clock]
            volumes = [
                sum(
                    (
                        quote_volumes[instrument][clock[day] + hour * HOUR]
        ...
        values = [
            (points[index + 1] / points[index]).ln()
            for index in range(SIGNAL_LOOKBACK_COMPLETE_DAYS)
        ]
        if not all(value.is_finite() for value in values) or not all(
            volume.is_finite() and volume > 0 for volume in volumes
        ):
            raise C14AHistoricalIndependentError("independent log return is invalid")
        returns[instrument] = values
        daily_quote_volumes[instrument] = volumes
    rows = []
    for instrument in selected:
        daily = returns[instrument]
        volumes = daily_quote_volumes[instrument]
        mean_absolute_return = sum((abs(value) for value in daily), ZERO) / Decimal(
            len(daily)
        )
        mean_quote_volume = sum(volumes, ZERO) / Decimal(len(volumes))
        inverse_quote_volume = ONE / mean_quote_volume
        ...
        if not all(value.is_finite() for value in (*daily, *volumes, score)):
            raise C14AHistoricalIndependentError("independent signal is non-finite")
        rows.append(
            {
...
def _simulate(
    *,
    ...
    def record(timestamp: datetime, event: str) -> None:
        path.append(equity)
        path_rows.append(
            {"timestamp": iso(timestamp), "event": event, "equity": equity}
        )
    ...
    while current < window.end_exclusive:
        ...
        if pending_close:
            ...
            record(current, "FORCED_OPEN_MARK")
            if equity <= 0:
                raise C14AHistoricalIndependentError(
                    "independent equity is non-positive before forced close"
                )
            changed = ZERO
            for instrument in selected:
            ...
            pending_close = False
            forced_closes += 1
            record(current, "FORCED_CLOSE")
            suppress = scheduled
            suppressed += int(suppress)
        if scheduled and not suppress:
            for instrument in selected:
                mark_to(instrument, trade[instrument][current])
            record(current, "SCHEDULED_OPEN_MARK")
            before = equity
            if before <= 0:
                raise C14AHistoricalIndependentError(
            ...
            record(current, "SCHEDULED_REBALANCE")
        ...
        if not equity.is_finite() or equity <= 0:
            raise C14AHistoricalIndependentError("independent equity is invalid")
        check_buffer()
        record(current + HOUR, "HOURLY_MARK")
        current += HOUR
    for instrument in selected:
        mark_to(instrument, trade[instrument][window.end_exclusive])
    record(window.end_exclusive, "TERMINAL_OPEN_MARK")
    if equity <= 0:
        raise C14AHistoricalIndependentError(
            "independent terminal pretrade equity is non-positive"
        )
    changed = ZERO
    for instrument in selected:
    ...
    turnover += changed
    weekly_ends.append(equity)
    record(window.end_exclusive, "TERMINAL_CLOSE")
    expected_funding = {
        (stamp, instrument)
        for stamp, values in source_funding.items()
    ...
    if (
        ...
        raise C14AHistoricalIndependentError("independent weekly coverage drift")
    weekly_pnl = [
    ...
    return len(left) == len(right) and all(
        ...
        and all(
            ...
            for field in (
                ...
                "mean_quote_volume",
                "inverse_quote_volume",
                "amihud_illiquidity",
                "ranking_score",
            )
        )
        and all(
            len(expected_row.get(vector, ())) == len(observed_row.get(vector, ()))
            and all(
                _close(expected_value, observed_value)
                for expected_value, observed_value in zip(
                    expected_row.get(vector, ()),
                    observed_row.get(vector, ()),
                    strict=True,
                )
            )
            for vector in (
...
def _review_replay(expected: Mapping[str, Any], retained: Mapping[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "complete_path_recompute": len(retained_path)
        == len(expected["path_rows"])
        and all(
            left["timestamp"] == right.get("timestamp")
            and left["event"] == right.get("event")
            and _close(left["equity"], right.get("equity"))
            for left, right in zip(
                expected["path_rows"], retained_path, strict=True
            )
        ),
...
def review_historical_window(
    ...
    *,
    ...
    if (
        ...
        raise C14AHistoricalIndependentError("independent window inventory drift")
    trade = {
    ...
    quote_volumes = {
        instrument: _series(
            ...
            "volume_quote",
            f"{instrument} trade quote volume",
        )
        for instrument in selected
    }
    marks = {
    ...
    for instrument in selected:
        ...
        _require_complete_clock(
            ...
            * HOUR,
            end_inclusive=window.end_exclusive,
            label=f"{instrument} trade quote volume",
        )
        _require_complete_clock(
            marks[instrument],
    ...
    for policy in POLICIES:
        if set(producer["replays"][policy]) != set(COST_RATES):
            raise C14AHistoricalIndependentError("independent cost inventory drift")
    ...
    return {
        "schema_version": 1,
        "stage": "C14A_WINDOW_INDEPENDENT_RECOMPUTE",
        "window_id": window_id,
...
def review_pooled_summary(
    ...
    if set(windows) != expected_ids:
        raise C14AHistoricalIndependentError("independent pooled inventory drift")
    ...
    for policy in POLICIES:
        ...
        for cost_label in COST_RATES:
            ...
            if (
                ...
                raise C14AHistoricalIndependentError(
                    "independent mean weekly equity coverage is invalid"
                )
            mean_weekly_equity = sum(weekly_equities, ZERO) / Decimal(
                len(weekly_equities)
            )
            ...
            reference[policy][cost_label] = {
                ...
                "mean_weekly_equity": mean_weekly_equity,
                "total_absolute_changed_notional": total_absolute_changed_notional,
    ...
    mean_absolute_return = reference["MeanAbsoluteReturnRankComparator"]["1.0x"]
    inverse_quote_volume = reference["InverseQuoteVolumeRankComparator"]["1.0x"]
    btc = [
        Decimal(str(value))
        for window in HISTORICAL_WINDOWS
    ...
    if btc_variance <= 0:
        raise C14AHistoricalIndependentError("independent pooled BTC variance is zero")
    beta = sum(
        (
    ...
    gates = {
        ...
        "absolute_btc_beta": abs(beta) <= Decimal("0.15"),
        ...
        == EXPECTED_NONFLAT_DIRECTIONS,
        "annualized_one_way_turnover": candidate["turnover"] <= Decimal("30.0"),
        ...
        "return_delta_vs_mean_absolute_return": candidate["aggregate_return"]
        - mean_absolute_return["aggregate_return"]
        > 0,
        "sharpe_delta_vs_mean_absolute_return": (
            candidate_sharpe - mean_absolute_return_sharpe
        )
        >= Decimal("0.10"),
        "drawdown_no_worse_than_mean_absolute_return": candidate["drawdown"]
        <= mean_absolute_return["drawdown"],
        "turnover_no_greater_than_mean_absolute_return": candidate["turnover"]
        <= mean_absolute_return["turnover"],
        "return_delta_vs_inverse_quote_volume": candidate["aggregate_return"]
        - inverse_quote_volume["aggregate_return"]
        > 0,
        "sharpe_delta_vs_inverse_quote_volume": (
            candidate_sharpe - inverse_quote_volume_sharpe
        )
        >= Decimal("0.10"),
        "drawdown_no_worse_than_inverse_quote_volume": candidate["drawdown"]
        <= inverse_quote_volume["drawdown"],
        "turnover_no_greater_than_inverse_quote_volume": candidate["turnover"]
        <= inverse_quote_volume["turnover"],
    }
    pooled_match = True
        ...
        for cost_label, expected in by_cost.items():
            ...
            pooled_match &= all(
                (
                    ...
                    _close(
                        observed.get("mean_weekly_equity"),
                        expected["mean_weekly_equity"],
                    ),
                    _close(
                        observed.get("total_absolute_changed_notional"),
                        expected["total_absolute_changed_notional"],
                    ),
                    _close(
    ...
    return {
        "schema_version": 1,
        "stage": "C14A_POOLED_INDEPENDENT_RECOMPUTE",
        "status": "PASS" if passed else "FAIL",
...
__all__ = [
    "C14AHistoricalIndependentError",
