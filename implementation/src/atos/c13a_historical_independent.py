"""Physically separate C13A signal, ledger, metric, and gate recomputation."""
...
class C13AHistoricalIndependentError(RuntimeError):
...
def _time(value: Any) -> datetime:
    try:
        ...
        raise C13AHistoricalIndependentError(f"invalid timestamp: {value!r}") from exc
    if result.tzinfo is None:
        raise C13AHistoricalIndependentError("timestamp must be timezone-aware")
    return result.astimezone(UTC)


def _number(value: Any, label: str, *, positive: bool = False) -> Decimal:
    if isinstance(value, bool):
        raise C13AHistoricalIndependentError(f"{label} must be decimal")
    try:
        result = Decimal(str(value))
    except (InvalidOperation, ValueError) as exc:
        raise C13AHistoricalIndependentError(f"{label} must be decimal") from exc
    if not result.is_finite() or (positive and result <= 0):
        raise C13AHistoricalIndependentError(f"{label} is non-finite or non-positive")
    return result
...
def _series(
    ...
    for row in rows:
        ...
        if stamp.minute or stamp.second or stamp.microsecond:
            raise C13AHistoricalIndependentError(f"{label} is off-grid")
        if last is not None and stamp <= last:
            raise C13AHistoricalIndependentError(f"{label} is duplicate or unordered")
        last = stamp
        output[stamp] = _number(row.get(field), f"{label} {field}", positive=True)
    if not output:
        raise C13AHistoricalIndependentError(f"{label} is empty")
    return output
...
def _require_funding_coverage(
    ...
    *,
    ...
    if not stamps:
        raise C13AHistoricalIndependentError(f"{label} funding coverage is empty")
    gaps = (
        stamps[0] - start,
    ...
    if any(gap < timedelta(0) or gap > maximum_gap for gap in gaps):
        raise C13AHistoricalIndependentError(f"{label} funding coverage drift")
    ...
    *,
    ...
    for instrument in selected:
        ...
        if not all(math.isfinite(value) for value in values):
            raise C13AHistoricalIndependentError("independent log return is invalid")
    ...
    for instrument in selected:
        ...
        total_volatility = math.sqrt(
            sum((value - daily_mean) ** 2 for value in daily) / (len(daily) - 1)
        )
        maximum_daily_return = max(daily)
        raw_seven_day_return = sum(daily)
        score = {
        ...
        if not all(math.isfinite(value) for value in (*daily, score)):
            raise C13AHistoricalIndependentError("independent signal is non-finite")
        rows.append(
            {
    ...
    *,
    ...
    while current < window.end_exclusive:
        ...
        if scheduled and not suppress:
            ...
            signal = _signal(
                current, selected=selected, trades=trade, policy=policy
            )
        ...
        if not equity.is_finite() or equity <= 0:
            raise C13AHistoricalIndependentError("independent equity is invalid")
        check_buffer()
    ...
    if (
        ...
        raise C13AHistoricalIndependentError("independent weekly coverage drift")
    weekly_pnl = [
    ...
    return len(left) == len(right) and all(
        ...
        and all(
            ...
            for field in (
                ...
                "total_volatility",
                "raw_seven_day_return",
                "ranking_score",
            )
        )
        and len(expected_row.get("daily_log_returns", ()))
        == len(observed_row.get("daily_log_returns", ()))
        and all(
            _close(expected_value, observed_value)
            for expected_value, observed_value in zip(
                expected_row.get("daily_log_returns", ()),
                observed_row.get("daily_log_returns", ()),
                strict=True,
            )
        )
...
def review_historical_window(
    ...
    *,
    ...
    if (
        ...
        raise C13AHistoricalIndependentError("independent window inventory drift")
    trade = {
    ...
    for policy in POLICIES:
        if set(producer["replays"][policy]) != set(COST_RATES):
            raise C13AHistoricalIndependentError("independent cost inventory drift")
    ...
    return {
        "schema_version": 1,
        "stage": "C13A_WINDOW_INDEPENDENT_RECOMPUTE",
        "window_id": window_id,
...
def review_pooled_summary(
    ...
    if set(windows) != expected_ids:
        raise C13AHistoricalIndependentError("independent pooled inventory drift")
    ...
    total_volatility = reference["TotalVolatilityRankComparator"]["1.0x"]
    raw_reversal = reference["RawSevenDayReversalComparator"]["1.0x"]
    btc = [
        Decimal(str(value))
        for window in HISTORICAL_WINDOWS
    ...
    if btc_variance <= 0:
        raise C13AHistoricalIndependentError("independent pooled BTC variance is zero")
    beta = sum(
        (
    ...
    gates = {
        ...
        "return_delta_vs_total_volatility": candidate["aggregate_return"]
        - total_volatility["aggregate_return"]
        > 0,
        "sharpe_delta_vs_total_volatility": (
            candidate_sharpe - total_volatility_sharpe
        )
        >= Decimal("0.10"),
        "drawdown_no_worse_than_total_volatility": candidate["drawdown"]
        <= total_volatility["drawdown"],
        "turnover_no_greater_than_total_volatility": candidate["turnover"]
        <= total_volatility["turnover"],
        "return_delta_vs_raw_reversal": candidate["aggregate_return"]
        - raw_reversal["aggregate_return"]
        > 0,
        "sharpe_delta_vs_raw_reversal": candidate_sharpe - raw_reversal_sharpe
        >= Decimal("0.10"),
        "drawdown_no_worse_than_raw_reversal": candidate["drawdown"]
        <= raw_reversal["drawdown"],
        "turnover_no_greater_than_raw_reversal": candidate["turnover"]
        <= raw_reversal["turnover"],
    }
    pooled_match = True
    ...
    return {
        "schema_version": 1,
        "stage": "C13A_POOLED_INDEPENDENT_RECOMPUTE",
        "status": "PASS" if passed else "FAIL",
