"""Frozen C10A cross-sectional residual-momentum research contract."""
...
CANDIDATE_ID = "C10ACrossSectionalResidualMomentum"
...
MARK_WARMUP_START = datetime(2023, 10, 8, 22, tzinfo=UTC)
...
class C10AContractError(RuntimeError):
    """Raised when a frozen C10A identity or time boundary drifts."""


def iso(value: datetime) -> str:
    if value.tzinfo is None:
        raise C10AContractError("C10A timestamp must be timezone-aware")
...
def window_by_id(window_id: str) -> HistoricalWindow:
    ...
    if len(matches) != 1:
        raise C10AContractError(f"unknown C10A window: {window_id!r}")
    return matches[0]
...
def decision_times(window: HistoricalWindow | str) -> tuple[datetime, ...]:
    ...
    while current < selected.end_exclusive:
        if current.weekday() != 0 or any(
            ...
            raise C10AContractError("C10A decision must be Monday 00:00 UTC")
        output.append(current)
        current += WEEK
    if len(output) != EXPECTED_DECISIONS_PER_WINDOW or current != selected.end_exclusive:
        raise C10AContractError("each C10A window must contain exactly 26 weeks")
    return tuple(output)
...
def validate_contract() -> dict[str, Any]:
    ...
    if report.get("familywise_trial_count") != EXPECTED_FAMILYWISE_TRIALS:
        raise C10AContractError("C10A program trial-count drift")
    previous = None
    decisions = 0
