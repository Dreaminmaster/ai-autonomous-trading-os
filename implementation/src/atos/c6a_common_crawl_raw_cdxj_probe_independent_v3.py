"""Independent reviewer for corrected raw Common Crawl CDXJ block semantics.

This reviewer imports no producer, transport, remote-search, block-selection, or
probe execution code. It reuses only generic retained-file parsing helpers from
the prior independent reviewer and independently treats the fifth cluster.idx
field as an opaque positive block ordinal, never as a CDXJ row count.
"""
...
def _parse_cluster(
    ...
    try:
        ...
        return None
    if offset < 0 or length <= 0 or block_ordinal <= 0:
        return None
    return (
        first[0],
        first[1],
...
def _expected_block_selection(
    ...
    if (
        first_row is None
        or f"{first_row[0]} {first_row[1]}" > low
        or upper_row is None
...
def review_probe(root: Path) -> dict[str, Any]:
    ...
    for query in queries:
        ...
        for block_index, block in enumerate(blocks):
            ...
            if (
                ...
                or block.get("block_ordinal") != block_ordinal
                or "record_count" in block
            ):
                errors.append(
            ...
            if (
                ...
                or not lines
                or len(lines) > MAX_CDXJ_LINES_PER_BLOCK
            ):
                errors.append(
                    f"block line-count bound mismatch: "
                    f"{query_id}:{block_index}"
                )
                continue
    ...
    if (
        result.get("cluster_fifth_field_semantics")
        != "OPAQUE_BLOCK_ORDINAL"
        or result.get("max_cdxj_lines_per_block")
        != MAX_CDXJ_LINES_PER_BLOCK
    ):
        errors.append("producer corrected block semantics missing")
