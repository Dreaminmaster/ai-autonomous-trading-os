"""Physically separate C11A signal, ledger, metric, and gate recomputation."""
...
class C11AHistoricalIndependentError(RuntimeError):
...
def review_formation_universe(
    ...
    if set(formation_rows) != set(CANDIDATE_POOL):
        raise C11AHistoricalIndependentError("independent formation inventory drift")
    scores = []
    ...
    for instrument in CANDIDATE_POOL:
        ...
        if (
            ...
            raise C11AHistoricalIndependentError(
    ...
    passed = (
        retained.get("stage") == "C11A_FORMATION_UNIVERSE"
    ...
    return {
        "schema_version": 1,
        "stage": "C11A_FORMATION_INDEPENDENT_RECOMPUTE",
...
def _time(value: Any) -> datetime:
    try:
        ...
        raise C11AHistoricalIndependentError(f"invalid timestamp: {value!r}") from exc
    if result.tzinfo is None:
        raise C11AHistoricalIndependentError("timestamp must be timezone-aware")
    return result.astimezone(UTC)


def _number(value: Any, label: str, *, positive: bool = False) -> Decimal:
    if isinstance(value, bool):
        raise C11AHistoricalIndependentError(f"{label} must be decimal")
    try:
        result = Decimal(str(value))
    except (InvalidOperation, ValueError) as exc:
        raise C11AHistoricalIndependentError(f"{label} must be decimal") from exc
    if not result.is_finite() or (positive and result <= 0):
        raise C11AHistoricalIndependentError(f"{label} is non-finite or non-positive")
    return result
...
def _series(
    ...
    for row in rows:
        ...
        if stamp.minute or stamp.second or stamp.microsecond:
            raise C11AHistoricalIndependentError(f"{label} is off-grid")
        if last is not None and stamp <= last:
            raise C11AHistoricalIndependentError(f"{label} is duplicate or unordered")
        last = stamp
        output[stamp] = _number(row.get(field), f"{label} {field}", positive=True)
    if not output:
        raise C11AHistoricalIndependentError(f"{label} is empty")
    return output
...
def _funding(
    ...
    if not output:
        raise C11AHistoricalIndependentError(f"{label} funding is empty")
    return tuple(output)
...
def _require_funding_coverage(
    ...
    *,
    ...
    if not stamps:
        raise C11AHistoricalIndependentError(f"{label} funding coverage is empty")
    gaps = (
        stamps[0] - start,
    ...
    if any(gap < timedelta(0) or gap > maximum_gap for gap in gaps):
        raise C11AHistoricalIndependentError(f"{label} funding coverage drift")
...
def _signal(
    ...
    *,
    ...
    last = stamp - 2 * HOUR
    first = last - REGRESSION_LOOKBACK_RETURNS * HOUR
    clock = [
        first + offset * HOUR
    ...
    for instrument in selected:
        ...
        if not all(math.isfinite(value) for value in values):
            raise C11AHistoricalIndependentError("independent log return is invalid")
    ...
    for instrument in selected:
        ...
        if not math.isfinite(denominator) or denominator <= 0:
            raise C11AHistoricalIndependentError("independent OLS variance is invalid")
        beta = sum(
        ...
        total_volatility = math.sqrt(
            sum((value - mean_y) ** 2 for value in dependent)
            / (len(dependent) - 1)
        )
        idiosyncratic_volatility = math.sqrt(
        ...
        if not all(
            ...
            raise C11AHistoricalIndependentError("independent signal is non-finite")
        rows.append(
            {
                "instrument": instrument,
                "alpha": alpha,
                "beta": beta,
                "total_volatility_score": total_volatility,
                "idiosyncratic_volatility_score": idiosyncratic_volatility,
                "ranking_score": score,
            }
        )
    ...
    *,
    ...
    while current < window.end_exclusive:
        ...
        if not equity.is_finite() or equity <= 0:
            raise C11AHistoricalIndependentError("independent equity is invalid")
        check_buffer()
    ...
    if (
        ...
        raise C11AHistoricalIndependentError("independent weekly coverage drift")
    weekly_pnl = [
...
def review_historical_window(
    ...
    *,
    ...
    if (
        ...
        raise C11AHistoricalIndependentError("independent window inventory drift")
    trade = {
    ...
    for instrument in selected:
        ...
        _require_complete_clock(
            marks[instrument],
            start=window.start - (REGRESSION_LOOKBACK_RETURNS + 2) * HOUR,
    ...
    for policy in POLICIES:
        if set(producer["replays"][policy]) != set(COST_RATES):
            raise C11AHistoricalIndependentError("independent cost inventory drift")
    ...
    return {
        "schema_version": 1,
        "stage": "C11A_WINDOW_INDEPENDENT_RECOMPUTE",
        "window_id": window_id,
        "status": "PASS" if passed else "FAIL",
...
def review_pooled_summary(
    ...
    if set(windows) != expected_ids:
        raise C11AHistoricalIndependentError("independent pooled inventory drift")
    ...
    total_volatility = reference["TotalVolatilityComparator"]["1.0x"]
    btc = [
        Decimal(str(value))
        for window in HISTORICAL_WINDOWS
    ...
    if btc_variance <= 0:
        raise C11AHistoricalIndependentError("independent pooled BTC variance is zero")
    beta = sum(
        (
    ...
    return {
        "schema_version": 1,
        "stage": "C11A_POOLED_INDEPENDENT_RECOMPUTE",
        "status": "PASS" if passed else "FAIL",
