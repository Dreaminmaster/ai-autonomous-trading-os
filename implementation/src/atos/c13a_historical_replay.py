"""Deterministic C13A signal, continuous-notional ledger, and replay."""
...
class C13AHistoricalReplayError(RuntimeError):
...
def _time(value: Any) -> datetime:
    try:
        ...
        raise C13AHistoricalReplayError(f"invalid timestamp: {value!r}") from exc
    if stamp.tzinfo is None:
        raise C13AHistoricalReplayError("timestamp must be timezone-aware")
    return stamp.astimezone(UTC)


def _decimal(value: Any, label: str, *, positive: bool = False) -> Decimal:
    if isinstance(value, bool):
        raise C13AHistoricalReplayError(f"{label} must be decimal")
    try:
        result = Decimal(str(value))
    except (InvalidOperation, ValueError) as exc:
        raise C13AHistoricalReplayError(f"{label} must be decimal") from exc
    if not result.is_finite() or (positive and result <= 0):
        qualifier = "positive finite" if positive else "finite"
        raise C13AHistoricalReplayError(f"{label} must be {qualifier}")
    return result
...
def _hourly_prices(
    ...
    *,
    ...
    if set(rows_by_instrument) != set(selected) or len(selected) != len(set(selected)):
        raise C13AHistoricalReplayError(f"{label} instrument inventory drift")
    ...
    for instrument in selected:
        ...
        for row in rows:
            ...
            if stamp.minute or stamp.second or stamp.microsecond:
                raise C13AHistoricalReplayError(f"{label} timestamp is off-grid")
            if previous is not None and stamp <= previous:
                raise C13AHistoricalReplayError(f"{label} rows are duplicate or unordered")
            previous = stamp
        ...
        if not timestamps or (reference is not None and timestamps != reference):
            raise C13AHistoricalReplayError(f"{label} timestamps are empty or misaligned")
...
def _funding_rows(
    ...
    *,
    ...
    if set(rows_by_instrument) != set(selected):
        raise C13AHistoricalReplayError("funding instrument inventory drift")
    ...
    for instrument in selected:
        ...
        for row in rows_by_instrument[instrument]:
            ...
            if previous is not None and stamp <= previous:
                raise C13AHistoricalReplayError(
            ...
            if instrument in by_instrument:
                raise C13AHistoricalReplayError("duplicate funding settlement")
...
def build_signal(
    ...
    *,
    ...
    """Rank seven complete UTC-day returns known 24 hours before execution."""
    ...
    if (
        ...
        raise C13AHistoricalReplayError("signal policy or selected universe drift")
    ...
    if decision.weekday() != 0 or any(
        ...
        raise C13AHistoricalReplayError("signal decision is not Monday 00:00 UTC")
    ...
    for instrument in selected:
        ...
        try:
            ...
            raise C13AHistoricalReplayError(
                f"signal missing exact trade open: {instrument} {iso(exc.args[0])}"
            ) from exc
        values = tuple(
            math.log(float(right / left))
            for left, right in pairwise(points)
        )
        if len(values) != SIGNAL_LOOKBACK_COMPLETE_DAYS or not all(
            math.isfinite(value) for value in values
        ):
            raise C13AHistoricalReplayError("signal return vector is invalid")
        returns[instrument] = values

    rows: list[dict[str, Any]] = []
    for instrument in selected:
        daily = returns[instrument]
        maximum_daily_return = max(daily)
        total_volatility = stdev(daily)
        raw_seven_day_return = math.fsum(daily)
        score = {
            CANDIDATE_ID: maximum_daily_return,
            "TotalVolatilityRankComparator": total_volatility,
            "RawSevenDayReversalComparator": raw_seven_day_return,
        }[policy]
        if not all(math.isfinite(value) for value in (*daily, score)):
            raise C13AHistoricalReplayError("signal statistic is non-finite")
        rows.append(
            {
                "instrument": instrument,
                "daily_log_returns": list(daily),
                "maximum_daily_return": maximum_daily_return,
                "total_volatility": total_volatility,
                "raw_seven_day_return": raw_seven_day_return,
                "ranking_score": score,
            }
        )
    ...
    if set(longs) & set(shorts) or len(longs) != LONG_COUNT or len(shorts) != SHORT_COUNT:
        raise C13AHistoricalReplayError("signal long/short rank construction failed")
    directions = {
...
def _maximum_drawdown(values: Sequence[Decimal]) -> Decimal:
    if not values or any(not value.is_finite() or value <= 0 for value in values):
        raise C13AHistoricalReplayError("drawdown path is empty or invalid")
    peak = values[0]
    maximum = Decimal(0)
    for value in values:
        peak = max(peak, value)
...
def evaluate_historical_window(
    ...
    *,
    ...
    if selected_window not in HISTORICAL_WINDOWS:
        raise C13AHistoricalReplayError("window is not frozen C13A history")
    selected = tuple(selected_universe)
    if selected != CANDIDATE_POOL:
        raise C13AHistoricalReplayError("selected universe must equal fixed instruments")
    if cost_label not in COST_RATES or policy not in POLICIES:
        raise C13AHistoricalReplayError("unknown C13A policy or cost")
    ...
    while current < selected_window.end_exclusive:
        ...
        if is_decision:
            if weekly_start is not None:
                ...
                if weekly_start_time is None:
                    raise C13AHistoricalReplayError("weekly start time is missing")
        ...
        if pending_forced_close:
            ...
            if equity <= 0:
                raise C13AHistoricalReplayError("non-positive equity before forced close")
        ...
        if is_decision and not suppress_decision:
            ...
            if pretrade_equity <= 0 or equity <= 0:
                raise C13AHistoricalReplayError("non-positive pretrade equity")
            pretrade_equity = equity
            signal = build_signal(
                current,
                selected_universe=selected,
            ...
            if abs(signed_target) > RECONCILIATION_TOLERANCE:
                raise C13AHistoricalReplayError("dollar-neutral target drift")
            expected_gross = GROSS_NOTIONAL * pretrade_equity
            if abs(gross_target - expected_gross) > RECONCILIATION_TOLERANCE:
                raise C13AHistoricalReplayError("gross target drift")
            turnover_sum += changed_notional
        ...
        if not equity.is_finite() or equity <= 0:
            raise C13AHistoricalReplayError("hourly equity is non-positive or non-finite")
        observe_buffer()
        record_event(current + HOUR, "HOURLY_MARK")
        current += HOUR

    if weekly_start is None or weekly_start_time is None:
        raise C13AHistoricalReplayError("window produced no weekly start")
    changed_notional = Decimal(0)
    for instrument in selected:
    ...
    if equity <= 0:
        raise C13AHistoricalReplayError("non-positive terminal pretrade equity")
    for instrument in selected:
        boundary_open = trades[instrument][selected_window.end_exclusive]
        delta = -quantities[instrument]
    ...
    if abs(residual) > RECONCILIATION_TOLERANCE or any(quantities.values()):
        raise C13AHistoricalReplayError("window accounting reconciliation failure")
    return {
        "stage": "C13A_HISTORICAL_WINDOW",
...
def _btc_weekly_mark_returns(
    ...
    for start in decision_times(window):
        ...
        if not value.is_finite():
            raise C13AHistoricalReplayError("BTC weekly benchmark return is invalid")
...
def evaluate_historical_window_matrix(
    ...
    *,
    ...
    if (
        ...
        raise C13AHistoricalReplayError("window matrix source inventory drift")
    ...
    return {
        "schema_version": 1,
        "stage": "C13A_HISTORICAL_WINDOW_MATRIX",
        "window": selected_window.to_dict(),
...
def _statistics(values: Sequence[Decimal]) -> dict[str, Any]:
    ...
    if len(raw) != EXPECTED_TOTAL_DECISIONS or not all(
        ...
        raise C13AHistoricalReplayError("pooled weekly return coverage is invalid")
...
def _pool_policy(
    ...
    if (
        ...
        raise C13AHistoricalReplayError("mean weekly equity coverage is invalid")
...
def _btc_beta(candidate: Sequence[Decimal], benchmark: Sequence[Decimal]) -> Decimal:
    if len(candidate) != EXPECTED_TOTAL_DECISIONS or len(benchmark) != len(candidate):
        raise C13AHistoricalReplayError("BTC beta return coverage is invalid")
    ...
    if variance <= 0 or not covariance.is_finite() or not variance.is_finite():
        raise C13AHistoricalReplayError("BTC beta regression is invalid")
    result = covariance / variance
    if not result.is_finite():
        raise C13AHistoricalReplayError("BTC beta is non-finite")
    return result


def summarize_h1_h5(windows: Mapping[str, Any]) -> dict[str, Any]:
    """Pool independent capital and apply every preregistered C13A gate."""

    expected_ids = {window.window_id for window in HISTORICAL_WINDOWS}
    if set(windows) != expected_ids:
        raise C13AHistoricalReplayError("C13A summary requires exactly H1-H5")
    selected_universes = {
        tuple(windows[window.window_id].get("selected_universe", ()))
        for window in HISTORICAL_WINDOWS
    }
    if selected_universes != {CANDIDATE_POOL}:
        raise C13AHistoricalReplayError("fixed universe drift across H1-H5")
    pooled = {
        policy: {
    ...
    total_volatility = pooled["TotalVolatilityRankComparator"]["1.0x"]
    raw_reversal = pooled["RawSevenDayReversalComparator"]["1.0x"]
    ...
    raw_reversal_sharpe = Decimal(
        str(raw_reversal["statistics"]["annualized_weekly_sharpe"])
    )
    gates = {
        ...
        "return_delta_vs_raw_reversal": Decimal(candidate["aggregate_return"])
        - Decimal(raw_reversal["aggregate_return"])
        > 0,
        "sharpe_delta_vs_raw_reversal": (
            candidate_sharpe - raw_reversal_sharpe
        )
        >= Decimal("0.10"),
        "drawdown_no_worse_than_raw_reversal": Decimal(
            candidate["maximum_drawdown"]
        )
        <= Decimal(raw_reversal["maximum_drawdown"]),
        "turnover_no_greater_than_raw_reversal": Decimal(
            candidate["annualized_one_way_turnover"]
        )
        <= Decimal(raw_reversal["annualized_one_way_turnover"]),
    }
    selected = all(gates.values())
    return {
        "schema_version": 1,
        "stage": "C13A_H1_H5_POOLED_SUMMARY",
