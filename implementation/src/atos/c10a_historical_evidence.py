"""Immutable C10A H1-H5 public-data and economic evidence package."""
...
from atos.c10a_historical_capture import validate_c10a_public_request
...
CONFIG_PATH = Path("implementation/config/c10a_cross_sectional_residual_momentum.json")
REGISTRY_PATH = Path("implementation/config/phase_c_research_program_registry_v1.json")
...
SHARED_RUNTIME_PATHS = (
    ...
    Path("implementation/src/atos/phase_c_research_program_guard.py"),
)
API_FAMILIES = {
...
class C10AHistoricalEvidenceError(RuntimeError):
    """Raised when final evidence cannot be proven complete and immutable."""


class C10AHistoricalDataEvidenceError(C10AHistoricalEvidenceError):
...
def _read_json(path: Path) -> Any:
    try:
        ...
        raise C10AHistoricalEvidenceError(f"invalid retained JSON: {path}") from exc
...
class C10AEvidencePackage:
    ...
    def write_json(self, relative: str, value: Any) -> None:
        if self._finalized:
            raise C10AHistoricalEvidenceError("evidence package is finalized")
        path = Path(relative)
        if path.is_absolute() or not path.parts or ".." in path.parts:
            raise C10AHistoricalEvidenceError("evidence path escapes package")
    ...
    def finalize(self, *, implementation_sha: str) -> dict[str, Any]:
        if not EXACT_SHA.fullmatch(implementation_sha):
            raise C10AHistoricalEvidenceError("implementation SHA must be exact")
        entries = sorted(self.root.rglob("*"))
        if any(path.is_symlink() for path in entries):
            raise C10AHistoricalEvidenceError("evidence contains a symbolic link")
        files = []
        ...
        if not files:
            raise C10AHistoricalEvidenceError("evidence package contains no files")
        manifest = {
            "schema_version": 1,
            "stage": "C10A_H1_H5_HISTORICAL_EVIDENCE_PACKAGE",
...
def verify_capture_package(
    ...
    except C10AHistoricalEvidenceError as exc:
        raise C10AHistoricalDataEvidenceError(str(exc)) from exc
...
def verify_evidence_package(
    ...
    if root.is_symlink() or any(path.is_symlink() for path in root.rglob("*")):
        raise C10AHistoricalEvidenceError("evidence package contains a symbolic link")
    manifest = _read_json(root / "manifest.json")
    if not isinstance(manifest, Mapping) or (
        manifest.get("stage") != "C10A_H1_H5_HISTORICAL_EVIDENCE_PACKAGE"
        or manifest.get("implementation_sha") != implementation_sha
        or any(
            manifest.get(key) != value for key, value in safety_boundary().items()
        )
    ):
        raise C10AHistoricalEvidenceError("evidence manifest identity drift")
    declared = manifest.get("files")
    if not isinstance(declared, list) or manifest.get("file_count") != len(declared):
        raise C10AHistoricalEvidenceError("evidence manifest inventory is invalid")
    observed = {
    ...
    if observed != declared_paths or len(declared_paths) != len(declared):
        raise C10AHistoricalEvidenceError("evidence manifest path inventory mismatch")
    for row in declared:
        if not isinstance(row, Mapping):
            raise C10AHistoricalEvidenceError("evidence manifest row is invalid")
...
def _verify_capture_package(root: Path, *, implementation_sha: str) -> dict[str, Any]:
    if root.is_symlink() or any(path.is_symlink() for path in root.rglob("*")):
        raise C10AHistoricalEvidenceError("capture package contains a symbolic link")
    manifest = _read_json(root / "manifest.json")
    ...
    if not all(isinstance(value, Mapping) for value in (manifest, index, universe)):
        raise C10AHistoricalEvidenceError("capture authority is not an object")
    if (
        manifest.get("stage") != "C10A_HISTORICAL_CAPTURE_PACKAGE"
        or index.get("stage") != "C10A_HISTORICAL_CAPTURE"
        or manifest.get("implementation_sha") != implementation_sha
        ...
        raise C10AHistoricalEvidenceError("capture identity, plan, or universe drift")
    if any(
        manifest.get(key) != value or index.get(key) != value
        for key, value in safety_boundary().items()
    ):
        raise C10AHistoricalEvidenceError("capture safety boundary drift")
    try:
        validate_checkout_binding(binding, implementation_sha=implementation_sha)
    except C10AHistoricalRunGuardError as exc:
        raise C10AHistoricalEvidenceError(str(exc)) from exc
    declared = manifest.get("files")
    if not isinstance(declared, list) or manifest.get("file_count") != len(declared):
        raise C10AHistoricalEvidenceError("capture manifest inventory is invalid")
    observed = {
        path.relative_to(root).as_posix()
        for path in root.rglob("*")
        if path.is_file() and path.name != "manifest.json"
    ...
    if observed != paths or len(paths) != len(declared):
        raise C10AHistoricalEvidenceError("capture manifest path inventory mismatch")
    for row in declared:
        if not isinstance(row, Mapping):
            raise C10AHistoricalEvidenceError("capture manifest row is invalid")
    ...
    if not isinstance(records, list) or not records:
        raise C10AHistoricalEvidenceError("capture request records are missing")
    request_ids = set()
    raw_paths = set()
    for record in records:
        if not isinstance(record, Mapping) or set(record) != {
            ...
            raise C10AHistoricalEvidenceError("capture request record schema drift")
        ...
        if (
            ...
            raise C10AHistoricalEvidenceError("capture request or raw path is duplicate")
        ...
        if relative != expected_relative or relative not in observed:
            raise C10AHistoricalEvidenceError("capture raw path identity drift")
        ...
        if (
            ...
            raise C10AHistoricalEvidenceError("capture record/raw digest mismatch")
        ...
        if (
            ...
            raise C10AHistoricalEvidenceError("capture retry provenance drift")
        try:
        ...
        if (
            ...
            raise C10AHistoricalEvidenceError("capture time or media type is invalid")
        ...
        try:
            validate_c10a_public_request(requested)
            validate_c10a_public_request(final)
        except C7APublicDataError as exc:
            raise C10AHistoricalEvidenceError(
                "capture request URL is outside public C10A policy"
            ) from exc
    ...
    if raw_paths != {path for path in observed if path.startswith("raw/")}:
        raise C10AHistoricalEvidenceError("capture raw record inventory mismatch")
    ...
    if (
        ...
        raise C10AHistoricalEvidenceError("capture selected universe is invalid")
    return {
...
def _rows(root: Path, series: str, instruments: tuple[str, ...]) -> dict[str, list[dict[str, Any]]]:
    ...
    for instrument in instruments:
        ...
        if (
            ...
            raise C10AHistoricalDataEvidenceError(
...
def _authority_hashes(root: Path, implementation_sha: str) -> dict[str, Any]:
    files = [
        path
        for path in (root / "implementation" / "src" / "atos").glob("c10a_*.py")
        if path.is_file()
    ]
    files.extend(
        path
        for path in (root / "implementation" / "scripts").glob("c10a_*.py")
        if path.is_file()
    )
    ...
    if any(not path.is_file() for path in files):
        raise C10AHistoricalEvidenceError("authority source inventory is incomplete")
    inventory = []
...
def build_h1_h5_evidence_package(
    ...
    *,
    ...
    if not EXACT_SHA.fullmatch(implementation_sha) or not SAFE_RUN_ID.fullmatch(
        ...
        raise C10AHistoricalEvidenceError("implementation SHA or run ID is invalid")
    try:
    ...
    if stamp.tzinfo is None:
        raise C10AHistoricalEvidenceError("evaluation time must be timezone-aware")
    ...
    final = {
        "schema_version": 1,
        "stage": "C10A_H1_H5_FINAL_CLASSIFICATION",
    ...
    package = C10AEvidencePackage(Path(output_root))
    package.write_json(
        "metadata.json",
        {
            "schema_version": 1,
            "stage": "C10A_H1_H5_HISTORICAL_ECONOMIC_RUN",
            "authoritative_run_id": authoritative_run_id,
            "implementation_sha": implementation_sha,
    ...
    if verification["verified_file_count"] != manifest["file_count"]:
        raise C10AHistoricalEvidenceError("post-finalization evidence audit drift")
