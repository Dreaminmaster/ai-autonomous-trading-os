"""Immutable C14A H1-H5 public-data and economic evidence package."""
...
from atos.c14a_historical_capture import validate_c14a_public_request
...
DESIGN_PATH = Path(
    "docs/architecture/phase-c/c14a-cross-sectional-liquidity-risk/"
    "C14A_CROSS_SECTIONAL_LIQUIDITY_RISK_CONTRACT_V1.md"
)
CONFIG_PATH = Path("implementation/config/c14a_cross_sectional_liquidity_risk.json")
REGISTRY_PATH = Path("implementation/config/phase_c_research_program_registry_v5.json")
...
SHARED_RUNTIME_PATHS = (
    ...
    Path("implementation/src/atos/c14a_research_program_guard.py"),
)
API_FAMILIES = {
...
class C14AHistoricalEvidenceError(RuntimeError):
    """Raised when final evidence cannot be proven complete and immutable."""


class C14AHistoricalDataEvidenceError(C14AHistoricalEvidenceError):
...
def _read_json(path: Path) -> Any:
    try:
        ...
        raise C14AHistoricalEvidenceError(f"invalid retained JSON: {path}") from exc
...
class C14AEvidencePackage:
    """Exclusive, atomic, no-overwrite evidence writer."""

    def __init__(self, root: Path):
        self.root = Path(root)
        if self.root.exists():
            raise C14AHistoricalEvidenceError(
    ...
    def write_json(self, relative: str, value: Any) -> None:
        if self._finalized:
            raise C14AHistoricalEvidenceError("evidence package is finalized")
        path = Path(relative)
        if path.is_absolute() or not path.parts or ".." in path.parts:
            raise C14AHistoricalEvidenceError("evidence path escapes package")
    ...
    def finalize(self, *, implementation_sha: str) -> dict[str, Any]:
        if not EXACT_SHA.fullmatch(implementation_sha):
            raise C14AHistoricalEvidenceError("implementation SHA must be exact")
        entries = sorted(self.root.rglob("*"))
        if any(path.is_symlink() for path in entries):
            raise C14AHistoricalEvidenceError("evidence contains a symbolic link")
        files = []
        ...
        if not files:
            raise C14AHistoricalEvidenceError("evidence package contains no files")
        manifest = {
            "schema_version": 1,
            "stage": "C14A_H1_H5_HISTORICAL_EVIDENCE_PACKAGE",
...
def verify_capture_package(
    ...
    except C14AHistoricalEvidenceError as exc:
        raise C14AHistoricalDataEvidenceError(str(exc)) from exc
...
def verify_evidence_package(
    ...
    if root.is_symlink() or any(path.is_symlink() for path in root.rglob("*")):
        raise C14AHistoricalEvidenceError("evidence package contains a symbolic link")
    manifest = _read_json(root / "manifest.json")
    if not isinstance(manifest, Mapping) or (
        manifest.get("stage") != "C14A_H1_H5_HISTORICAL_EVIDENCE_PACKAGE"
        or manifest.get("implementation_sha") != implementation_sha
        or any(
            manifest.get(key) != value for key, value in safety_boundary().items()
        )
    ):
        raise C14AHistoricalEvidenceError("evidence manifest identity drift")
    declared = manifest.get("files")
    if not isinstance(declared, list) or manifest.get("file_count") != len(declared):
        raise C14AHistoricalEvidenceError("evidence manifest inventory is invalid")
    observed = {
    ...
    if observed != declared_paths or len(declared_paths) != len(declared):
        raise C14AHistoricalEvidenceError("evidence manifest path inventory mismatch")
    for row in declared:
        if not isinstance(row, Mapping):
            raise C14AHistoricalEvidenceError("evidence manifest row is invalid")
...
def _verify_capture_package(root: Path, *, implementation_sha: str) -> dict[str, Any]:
    if root.is_symlink() or any(path.is_symlink() for path in root.rglob("*")):
        raise C14AHistoricalEvidenceError("capture package contains a symbolic link")
    manifest = _read_json(root / "manifest.json")
    ...
    if not all(isinstance(value, Mapping) for value in (manifest, index)):
        raise C14AHistoricalEvidenceError("capture authority is not an object")
    if (
        manifest.get("stage") != "C14A_HISTORICAL_CAPTURE_PACKAGE"
        or index.get("stage") != "C14A_HISTORICAL_CAPTURE"
        or manifest.get("implementation_sha") != implementation_sha
        ...
        raise C14AHistoricalEvidenceError("capture identity, plan, or universe drift")
    if any(
        manifest.get(key) != value or index.get(key) != value
        for key, value in safety_boundary().items()
    ):
        raise C14AHistoricalEvidenceError("capture safety boundary drift")
    try:
        validate_checkout_binding(binding, implementation_sha=implementation_sha)
    except C14AHistoricalRunGuardError as exc:
        raise C14AHistoricalEvidenceError(str(exc)) from exc
    declared = manifest.get("files")
    if not isinstance(declared, list) or manifest.get("file_count") != len(declared):
        raise C14AHistoricalEvidenceError("capture manifest inventory is invalid")
    observed = {
        path.relative_to(root).as_posix()
        for path in root.rglob("*")
        if path.is_file() and path.name != "manifest.json"
    ...
    if observed != paths or len(paths) != len(declared):
        raise C14AHistoricalEvidenceError("capture manifest path inventory mismatch")
    for row in declared:
        if not isinstance(row, Mapping):
            raise C14AHistoricalEvidenceError("capture manifest row is invalid")
    ...
    if not isinstance(records, list) or not records:
        raise C14AHistoricalEvidenceError("capture request records are missing")
    request_ids = set()
    raw_paths = set()
    for record in records:
        if not isinstance(record, Mapping) or set(record) != {
            ...
            raise C14AHistoricalEvidenceError("capture request record schema drift")
        ...
        if (
            ...
            raise C14AHistoricalEvidenceError("capture request or raw path is duplicate")
        ...
        if relative != expected_relative or relative not in observed:
            raise C14AHistoricalEvidenceError("capture raw path identity drift")
        ...
        if (
            ...
            raise C14AHistoricalEvidenceError("capture record/raw digest mismatch")
        ...
        if (
            ...
            raise C14AHistoricalEvidenceError("capture retry provenance drift")
        try:
        ...
        if (
            ...
            raise C14AHistoricalEvidenceError("capture time or media type is invalid")
        ...
        try:
            validate_c14a_public_request(requested)
            validate_c14a_public_request(final)
        except C7APublicDataError as exc:
            raise C14AHistoricalEvidenceError(
                "capture request URL is outside public C14A policy"
            ) from exc
    ...
    if raw_paths != {path for path in observed if path.startswith("raw/")}:
        raise C14AHistoricalEvidenceError("capture raw record inventory mismatch")
    selected = tuple(str(value) for value in index.get("selected_universe", ()))
    if (
        selected != CANDIDATE_POOL
    ):
        raise C14AHistoricalEvidenceError("capture selected universe is invalid")
    return {
...
def _rows(root: Path, series: str, instruments: tuple[str, ...]) -> dict[str, list[dict[str, Any]]]:
    ...
    for instrument in instruments:
        ...
        if (
            ...
            raise C14AHistoricalDataEvidenceError(
...
def _authority_hashes(root: Path, implementation_sha: str) -> dict[str, Any]:
    files = [
        path
        for path in (root / "implementation" / "src" / "atos").glob("c14a_*.py")
        if path.is_file()
    ]
    files.extend(
        path
        for path in (root / "implementation" / "scripts").glob("c14a_*.py")
        if path.is_file()
    )
    ...
    if any(not path.is_file() for path in files):
        raise C14AHistoricalEvidenceError("authority source inventory is incomplete")
    inventory = []
...
def build_h1_h5_evidence_package(
    ...
    *,
    ...
    if not EXACT_SHA.fullmatch(implementation_sha) or not SAFE_RUN_ID.fullmatch(
        ...
        raise C14AHistoricalEvidenceError("implementation SHA or run ID is invalid")
    try:
    ...
    if stamp.tzinfo is None:
        raise C14AHistoricalEvidenceError("evaluation time must be timezone-aware")
    ...
    package = C14AEvidencePackage(Path(output_root))
    package.write_json(
        "metadata.json",
        {
            "schema_version": 1,
            "stage": "C14A_H1_H5_HISTORICAL_ECONOMIC_RUN",
            "authoritative_run_id": authoritative_run_id,
            "implementation_sha": implementation_sha,
    ...
    if verification["verified_file_count"] != manifest["file_count"]:
        raise C14AHistoricalEvidenceError("post-finalization evidence audit drift")
    return final, manifest


__all__ = [
    "C14AEvidencePackage",
    "C14AHistoricalDataEvidenceError",
    "C14AHistoricalEvidenceError",
