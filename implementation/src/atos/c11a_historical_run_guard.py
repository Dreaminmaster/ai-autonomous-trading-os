"""Exact clean-checkout binding for the one-shot C11A authority run."""
...
class C11AHistoricalRunGuardError(RuntimeError):
...
def validate_checkout_binding(
    ...
    if not EXACT_SHA.fullmatch(implementation_sha) or dict(value) != expected:
        raise C11AHistoricalRunGuardError(
            "C11A checkout binding is dirty or SHA-mismatched"
        )
    return expected
...
def verify_checkout_binding(
    ...
    *,
    ...
    if not EXACT_SHA.fullmatch(implementation_sha):
        raise C11AHistoricalRunGuardError("implementation SHA must be exact")
    ...
    try:
        ...
        raise C11AHistoricalRunGuardError(
            "unable to verify the C11A Git checkout"
        ) from exc
    if head != implementation_sha or status:
        raise C11AHistoricalRunGuardError(
            "C11A authority requires its exact clean implementation checkout"
        )
