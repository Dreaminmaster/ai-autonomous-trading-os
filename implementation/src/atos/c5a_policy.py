"""C5A crowding signals, capped sizing, and post-cost solver."""
...
def _waterfill_weights(
    ...
    if len(active) < 2:
        raise C5AError("water-filling requires at least two eligible assets")
    ...
    while active:
        ...
        if denominator <= 0 or not math.isfinite(denominator):
            raise C5AError("invalid inverse-volatility denominator")
        provisional = {
        ...
        for spot in sorted(over):
            ...
            active.remove(spot)
            if remaining < -1e-12:
                raise C5AError("capped allocation over-assigned")
    if abs(sum(weights.values()) - float(config["total_invested_weight"])) > 1e-12:
        raise C5AError("capped allocation does not sum to invested target")
    if any(value < -1e-12 or value > cap + 1e-12 for value in weights.values()):
        raise C5AError("capped allocation violates asset bounds")
...
def signal_snapshot(
    ...
    *,
    ...
    if not isinstance(observations, Mapping):
        raise C5AError("invalid C5A calibration object")
    ...
    if risk_on and abs(sum(target_weights.values()) - 0.80) > 1e-12:
        raise C5AError("risk-on target does not invest exactly 80%")
    return {
...
def solve_post_cost(
    ...
    if equity_before <= 0 or fee_rate < 0:
        raise C5AError("invalid post-cost solver inputs")
    current = {
    ...
    if any(value < 0 for value in current.values()) or any(value < 0 for value in weights.values()):
        raise C5AError("negative current value or target weight")
    if sum(weights.values()) > 1.0 + 1e-12:
        raise C5AError("target weights exceed one")
    ...
    if equation(lower) > 1e-12 or equation(upper) < -1e-12:
        raise C5AError("post-cost root is not bracketed")
    midpoint = upper
    iterations = 0
    for iterations in range(1, 201):
        ...
        raise C5AError("post-cost root did not converge")
    ...
    if cash < -1e-12 or abs(residual) > 1e-9:
        raise C5AError("post-cost accounting identity failure")
    return {
...
def _weights(
    ...
    if not math.isfinite(equity) or equity <= 0:
        raise C5AError("invalid marked equity")
    ...
    if abs(sum(asset_weights.values()) + cash_weight - 1.0) > 1e-12:
        raise C5AError("current weights do not reconcile")
...
def one_way_distance(
    ...
    if not math.isfinite(result) or result < -1e-12:
        raise C5AError("invalid one-way distance")
