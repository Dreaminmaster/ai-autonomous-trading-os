...
from atos.time_context import resolve_decision_day, resolve_decision_ts
...
class RiskEngine:
    ...
    def evaluate(
        self, intent: dict[str, Any], state: dict[str, Any] | None = None
    ) -> RiskDecision:
        ...
        ks_active = bool(state.get("kill_switch_active")) or bool(
            flag_path and Path(flag_path).exists()
        )
        if not ks_active:
        ...
        if state.get("emergency_stop"):
            return RiskDecision(
                "risk_decision.v1", "PAUSED", ["emergency_stop_activated"], 1.0, checks
            )

        # ── Gate 3: Mode guard ──────────────────────────────────
        if mode == RunMode.LIVE.value and not state.get(
        ...
        checks["symbol_allowed"] = (
            symbol in allowed_symbols or action == Action.HOLD.value
        )
        ...
        min_conf = float(
            self.policy.get("ai_output_limits", {}).get(
        ...
        cooldown_sec = float(
            self.policy.get("trade_limits", {}).get("cooldown_seconds", 300)
        )
        ...
        if current_dd >= max_dd:
            return RiskDecision(
                "risk_decision.v1",
                "PAUSED",
                [f"max_drawdown_exceeded: {current_dd:.1f}% >= {max_dd}%"],
                1.0,
                checks,
            )
        ...
        prospective_gross = (
            gross_exposure + position_size_pct - 2.0 * offset
            if action != Action.HOLD.value
            else gross_exposure
        )
        prospective_symbol = (
            symbol_exposure + position_size_pct - 2.0 * offset
            if action != Action.HOLD.value
            else symbol_exposure
        )
        max_gross = float(
            self.policy.get("portfolio_limits", {}).get("max_gross_exposure_pct", 20.0)
        )
        max_symbol = float(
            self.policy.get("portfolio_limits", {}).get("max_symbol_exposure_pct", 5.0)
        )
        checks["portfolio_exposure"] = {
            "current_gross_pct": gross_exposure,
            "prospective_gross_pct": prospective_gross,
            "max_gross_pct": max_gross,
            "current_symbol_pct": symbol_exposure,
            "prospective_symbol_pct": prospective_symbol,
            "max_symbol_pct": max_symbol,
        }
        if action != Action.HOLD.value and prospective_gross > max_gross:
            reasons.append(
                f"gross_exposure_exceeds_limit: {prospective_gross:.4f} > {max_gross}"
            )
        ...
        if action == Action.HOLD.value and not reasons:
            ...
            return RiskDecision(
                "risk_decision.v1", "APPROVED", ["hold_is_safe_default"], 0.0, checks
            )
        ...
        return RiskDecision(
            "risk_decision.v1", "APPROVED", ["all_checks_passed"], 0.1, checks
        )

    # ── Quick pre-check ────────────────────────────────────────

    def quick_check(
        self, intent: dict, allowed: set[str] | None = None
    ) -> tuple[bool, str]:
    ...
    def stats(self) -> dict:
        return {
            ...
            "max_trades_in_single_day": max(self._daily_trades.values())
