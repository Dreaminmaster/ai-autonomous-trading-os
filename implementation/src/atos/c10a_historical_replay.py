"""Deterministic C10A signal, continuous-notional ledger, and replay."""
...
class C10AHistoricalReplayError(RuntimeError):
...
def _time(value: Any) -> datetime:
    try:
        ...
        raise C10AHistoricalReplayError(f"invalid timestamp: {value!r}") from exc
    if stamp.tzinfo is None:
        raise C10AHistoricalReplayError("timestamp must be timezone-aware")
    return stamp.astimezone(UTC)


def _decimal(value: Any, label: str, *, positive: bool = False) -> Decimal:
    if isinstance(value, bool):
        raise C10AHistoricalReplayError(f"{label} must be decimal")
    try:
        result = Decimal(str(value))
    except (InvalidOperation, ValueError) as exc:
        raise C10AHistoricalReplayError(f"{label} must be decimal") from exc
    if not result.is_finite() or (positive and result <= 0):
        qualifier = "positive finite" if positive else "finite"
        raise C10AHistoricalReplayError(f"{label} must be {qualifier}")
    return result
...
def _hourly_prices(
    ...
    *,
    ...
    if set(rows_by_instrument) != set(selected) or len(selected) != len(set(selected)):
        raise C10AHistoricalReplayError(f"{label} instrument inventory drift")
    ...
    for instrument in selected:
        ...
        for row in rows:
            ...
            if stamp.minute or stamp.second or stamp.microsecond:
                raise C10AHistoricalReplayError(f"{label} timestamp is off-grid")
            if previous is not None and stamp <= previous:
                raise C10AHistoricalReplayError(f"{label} rows are duplicate or unordered")
            previous = stamp
        ...
        if not timestamps or (reference is not None and timestamps != reference):
            raise C10AHistoricalReplayError(f"{label} timestamps are empty or misaligned")
...
def _funding_rows(
    ...
    *,
    ...
    if set(rows_by_instrument) != set(selected):
        raise C10AHistoricalReplayError("funding instrument inventory drift")
    ...
    for instrument in selected:
        ...
        for row in rows_by_instrument[instrument]:
            ...
            if instrument in by_instrument:
                raise C10AHistoricalReplayError("duplicate funding settlement")
...
def _ols(y: Sequence[float], x: Sequence[float]) -> tuple[float, float]:
    if len(y) != BETA_LOOKBACK_RETURNS or len(x) != len(y):
        raise C10AHistoricalReplayError("OLS observation count drift")
    ...
    if not math.isfinite(covariance) or not math.isfinite(variance) or variance <= 0:
        raise C10AHistoricalReplayError("OLS factor variance is zero or invalid")
    beta = covariance / variance
    alpha = mean_y - beta * mean_x
    if not math.isfinite(alpha) or not math.isfinite(beta):
        raise C10AHistoricalReplayError("OLS coefficient is non-finite")
    return alpha, beta
...
def build_signal(
    ...
    *,
    ...
    selected = tuple(selected_universe)
    if (
        policy not in {CANDIDATE_ID, "RawReturnMomentumComparator"}
        or len(selected) != 8
        or len(set(selected)) != 8
        or set(mark_closes) != set(selected)
    ):
        raise C10AHistoricalReplayError("signal policy or selected universe drift")
    ...
    if decision.weekday() != 0 or any(
        ...
        raise C10AHistoricalReplayError("signal decision is not Monday 00:00 UTC")
    last_stamp = decision - 2 * HOUR
    first_stamp = last_stamp - BETA_LOOKBACK_RETURNS * HOUR
    stamps = tuple(
        first_stamp + offset * HOUR for offset in range(BETA_LOOKBACK_RETURNS + 1)
    )
    ...
    for instrument in selected:
        ...
        if len(values) != BETA_LOOKBACK_RETURNS or not all(
            math.isfinite(value) for value in values
        ):
            raise C10AHistoricalReplayError("signal return vector is invalid")
    ...
    for instrument in selected:
        ...
        raw_score = math.fsum(own[-RESIDUAL_SCORE_RETURNS:])
        residual_score = math.fsum(
        ...
        score = residual_score if policy == CANDIDATE_ID else raw_score
        if not all(
            math.isfinite(value)
            for value in (alpha, beta, raw_score, residual_score, score)
        ):
            raise C10AHistoricalReplayError("signal statistic is non-finite")
        rows.append(
            {
    ...
    if set(longs) & set(shorts) or len(longs) != LONG_COUNT or len(shorts) != SHORT_COUNT:
        raise C10AHistoricalReplayError("signal long/short rank construction failed")
    directions = {
    ...
    return {
        ...
        "regression_return_count": BETA_LOOKBACK_RETURNS,
        "residual_score_return_count": RESIDUAL_SCORE_RETURNS,
        "rows": rows,
        "longs": list(longs),
        "shorts": list(shorts),
...
def _maximum_drawdown(values: Sequence[Decimal]) -> Decimal:
    if not values or any(not value.is_finite() or value <= 0 for value in values):
        raise C10AHistoricalReplayError("drawdown path is empty or invalid")
    peak = values[0]
    maximum = Decimal(0)
    for value in values:
        peak = max(peak, value)
...
def evaluate_historical_window(
    ...
    *,
    ...
    if selected_window not in HISTORICAL_WINDOWS:
        raise C10AHistoricalReplayError("window is not frozen C10A history")
    selected = tuple(selected_universe)
    if len(selected) != 8 or len(set(selected)) != 8:
        raise C10AHistoricalReplayError("selected universe must contain eight instruments")
    if cost_label not in COST_RATES or policy not in POLICIES:
        raise C10AHistoricalReplayError("unknown C10A policy or cost")
    ...
    _require_exact_hours(
        ...
        start=selected_window.start - (BETA_LOOKBACK_RETURNS + 2) * HOUR,
    ...
    while current < selected_window.end_exclusive:
        ...
        if is_decision:
            if weekly_start is not None:
                ...
                if weekly_start_time is None:
                    raise C10AHistoricalReplayError("weekly start time is missing")
        ...
        if pending_forced_close:
            ...
            if equity <= 0:
                raise C10AHistoricalReplayError("non-positive equity before forced close")
        ...
        if is_decision and not suppress_decision:
            ...
            if pretrade_equity <= 0 or equity <= 0:
                raise C10AHistoricalReplayError("non-positive pretrade equity")
            pretrade_equity = equity
            ...
            if policy != "AlwaysLongSelectedUniverseComparator" and abs(
                ...
                raise C10AHistoricalReplayError("dollar-neutral target drift")
            expected_gross = GROSS_NOTIONAL * pretrade_equity
            if abs(gross_target - expected_gross) > RECONCILIATION_TOLERANCE:
                raise C10AHistoricalReplayError("gross target drift")
            turnover_sum += changed_notional / pretrade_equity
        ...
        if not equity.is_finite() or equity <= 0:
            raise C10AHistoricalReplayError("hourly equity is non-positive or non-finite")
        observe_buffer()
        record_event(current + HOUR, "HOURLY_MARK")
        current += HOUR

    if weekly_start is None or weekly_start_time is None:
        raise C10AHistoricalReplayError("window produced no weekly start")
    changed_notional = Decimal(0)
    for instrument in selected:
    ...
    if equity <= 0:
        raise C10AHistoricalReplayError("non-positive terminal pretrade equity")
    ...
    if abs(residual) > RECONCILIATION_TOLERANCE or any(quantities.values()):
        raise C10AHistoricalReplayError("window accounting reconciliation failure")
    return {
        "stage": "C10A_HISTORICAL_WINDOW",
...
def _btc_weekly_mark_returns(
    ...
    for start in decision_times(window):
        ...
        if not value.is_finite():
            raise C10AHistoricalReplayError("BTC weekly benchmark return is invalid")
...
def evaluate_historical_window_matrix(
    ...
    *,
    ...
    if (
        ...
        raise C10AHistoricalReplayError("window matrix source inventory drift")
    ...
    return {
        "schema_version": 1,
        "stage": "C10A_HISTORICAL_WINDOW_MATRIX",
        "window": selected_window.to_dict(),
...
def _statistics(values: Sequence[Decimal]) -> dict[str, Any]:
    ...
    if len(raw) != EXPECTED_TOTAL_DECISIONS or not all(
        ...
        raise C10AHistoricalReplayError("pooled weekly return coverage is invalid")
...
def _btc_beta(candidate: Sequence[Decimal], benchmark: Sequence[Decimal]) -> Decimal:
    if len(candidate) != EXPECTED_TOTAL_DECISIONS or len(benchmark) != len(candidate):
        raise C10AHistoricalReplayError("BTC beta return coverage is invalid")
    ...
    if variance <= 0 or not covariance.is_finite() or not variance.is_finite():
        raise C10AHistoricalReplayError("BTC beta regression is invalid")
    result = covariance / variance
    if not result.is_finite():
        raise C10AHistoricalReplayError("BTC beta is non-finite")
    return result


def summarize_h1_h5(windows: Mapping[str, Any]) -> dict[str, Any]:
    """Pool independent capital and apply every preregistered C10A gate."""

    expected_ids = {window.window_id for window in HISTORICAL_WINDOWS}
    if set(windows) != expected_ids:
        raise C10AHistoricalReplayError("C10A summary requires exactly H1-H5")
    selected_universes = {
        tuple(windows[window.window_id].get("selected_universe", ()))
        for window in HISTORICAL_WINDOWS
    }
    if len(selected_universes) != 1:
        raise C10AHistoricalReplayError("selected universe drift across H1-H5")
    pooled = {
        policy: {
    ...
    raw = pooled["RawReturnMomentumComparator"]["1.0x"]
    ...
    gates = {
        ...
        "return_delta_vs_raw_momentum": Decimal(candidate["aggregate_return"])
        - Decimal(raw["aggregate_return"])
        > 0,
        "sharpe_delta_vs_raw_momentum": candidate_sharpe - raw_sharpe
        >= Decimal("0.10"),
        "drawdown_no_worse_than_raw_momentum": Decimal(candidate["maximum_drawdown"])
        <= Decimal(raw["maximum_drawdown"]),
        "turnover_no_greater_than_raw_momentum": Decimal(
            candidate["annualized_one_way_turnover"]
        )
        <= Decimal(raw["annualized_one_way_turnover"]),
    }
    selected = all(gates.values())
    return {
        "schema_version": 1,
        "stage": "C10A_H1_H5_POOLED_SUMMARY",
