...
from atos.c6a_common_crawl_raw_cdxj_core import (
    RESULT_FAILED, RESULT_VERIFIED, STAGE, ClusterBlock, ProbeError,
    RangeTransport, _canonical_json_bytes, _cdx_url, _cluster_url,
    _load_inventory, _validate_environment, atomic_write_json,
    exact_okx_surt, normalized_okx_url, parse_cdxj_line,
)
from atos.c6a_common_crawl_raw_cdxj_transport import (
    RecordingRangeReader, RemoteSortedLineIndex, UrllibRangeTransport,
)


def select_cluster_blocks(context: list[ClusterBlock], target_surt: str, maximum: int) -> tuple[list[ClusterBlock], str]:
    low = f"{target_surt} 00000000000000"
    high = f"{target_surt} 99999999999999"
    ordered = sorted(context, key=lambda row: row.sort_key)
    predecessor_indices = [i for i, row in enumerate(ordered) if row.sort_key <= low]
    ...
    for row in ordered[anchor:]:
        ...
        if len(selected) > maximum:
            raise ProbeError("exact URL spans more CDX blocks than frozen maximum")
    if not selected or not upper_boundary:
        raise ProbeError("cluster context did not prove an upper boundary")
...
def _query_one(
    ...
    index = RemoteSortedLineIndex(
        ...
        max_requests=int(inventory["max_cluster_range_requests_per_query"]),
    )
    ...
    selected, upper_boundary = select_cluster_blocks(
        context, target_surt, int(inventory["max_cdx_blocks_per_query"])
    )
    ...
    for block_index, block in enumerate(selected):
        ...
        try:
            ...
            raise ProbeError("selected CDX gzip block failed to decompress") from exc
        try:
            text = decompressed.decode("utf-8")
        except UnicodeDecodeError as exc:
            raise ProbeError("selected CDX block is not UTF-8") from exc
        lines = [line for line in text.splitlines() if line]
        if not lines:
            raise ProbeError("selected CDX block is empty")
        first_key, first_timestamp, _ = parse_cdxj_line(lines[0])
        if (first_key, first_timestamp) != (block.first_urlkey, block.first_timestamp):
            raise ProbeError("CDX block first row does not match cluster secondary index")
        relative = f"queries/{query_id}/block-{block_index:02d}.cdxj"
        ...
        for line in lines:
            ...
            try:
                ...
                continue
            if captured_url != target_url or str(payload.get("status")) != "200":
                continue
            ...
            try:
                ...
                raise ProbeError("matching CDX row lacks valid WARC locator")
            if (
                ...
                raise ProbeError("matching CDX row WARC locator escaped frozen crawl")
            exact_rows.append({
                "urlkey": urlkey,
                "timestamp": timestamp,
                "url": captured_url,
            ...
            block_exact += 1
            if len(exact_rows) > int(inventory["max_exact_rows_per_query"]):
                raise ProbeError("exact CDX row count exceeds frozen maximum")
        block_rows.append({
    ...
    atomic_write_json(output / f"queries/{query_id}/query.json", query)
    return query
...
def run_probe(
    ...
    *,
    ...
    (output / "inventory_snapshot.json").write_bytes(inventory_bytes)
    reader = RecordingRangeReader(
        transport or UrllibRangeTransport(max_bytes=int(inventory["max_cdx_block_bytes"])),
        output,
        minimum_interval_seconds=float(inventory["minimum_request_interval_seconds"]),
        sleeper=sleeper,
    )
    ...
    for target in inventory["targets"]:
        for crawl in target["crawl_indexes"]:
            try:
                queries.append(_query_one(target, crawl, reader, output, inventory))
            except ProbeError as exc:
                errors.append({
                    "query_id": f"{target['target_id']}--{crawl}",
                    "error_type": type(exc).__name__,
                    "error": str(exc),
                })
    atomic_write_json(output / "range_evidence.json", {
        "schema_version": 1,
        "stage": STAGE,
        "requests": reader.evidence,
        "request_count": len(reader.evidence),
    })
    hit_queries = [row["query_id"] for row in queries if row["exact_row_count"] > 0]
    result = {
        "schema_version": 1,
        "stage": STAGE,
        "inventory_sha256": hashlib.sha256(inventory_bytes).hexdigest(),
        "status": "PASS" if not errors and len(queries) == 23 else "FAIL",
        "result": RESULT_VERIFIED if not errors and len(queries) == 23 else RESULT_FAILED,
        "target_count": 7,
