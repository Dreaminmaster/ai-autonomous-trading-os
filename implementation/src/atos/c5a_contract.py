"""Frozen C5A contract, public-market validation, and calibration primitives.
...
def _timestamp(value: Any) -> datetime:
    if isinstance(value, datetime):
        ...
        raise C5AError(f"invalid timestamp: {value!r}")
    return result.replace(tzinfo=UTC) if result.tzinfo is None else result.astimezone(UTC)


def _finite(value: Any, label: str) -> float:
    if value is None or isinstance(value, bool):
        raise C5AError(f"{label} must be numeric")
    try:
        result = float(value)
    except (TypeError, ValueError) as exc:
        raise C5AError(f"{label} must be numeric") from exc
    if not math.isfinite(result):
        raise C5AError(f"{label} must be finite")
    return result
...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise C5AError("C5A safety state drift")
...
def expected_timestamps() -> tuple[datetime, ...]:
    ...
    if len(values) != 2940 or values[-1] != BOUNDARY - STEP:
        raise C5AError("internal C5A timestamp-grid mismatch")
    return values
...
def _rows_by_timestamp(
    ...
    *,
    ...
    if not rows:
        raise C5AError(f"empty series: {label}")
    ...
    if timestamps != tuple(sorted(timestamps)) or len(set(timestamps)) != len(timestamps):
        raise C5AError(f"unordered or duplicate timestamps: {label}")
    if timestamps != expected_timestamps():
        raise C5AError(f"exact retained coverage mismatch: {label}")
    ...
    for field in fields:
        ...
        if len(array) != len(timestamps) or not np.isfinite(array).all():
            raise C5AError(f"invalid series: {label} {field}")
        arrays[field] = array
...
def prepare_market(datasets: Mapping[str, Any]) -> C5AMarket:
    if set(datasets) != {"spot", "swap", "mark"}:
        raise C5AError("C5A dataset sections must be spot/swap/mark")
    spot = datasets["spot"]
    swap = datasets["swap"]
    mark = datasets["mark"]
    if not isinstance(spot, Mapping) or set(spot) != set(SPOT_INSTRUMENTS):
        raise C5AError("C5A spot dataset set mismatch")
    if not isinstance(swap, Mapping) or set(swap) != set(SWAP_INSTRUMENTS):
        raise C5AError("C5A swap dataset set mismatch")
    if not isinstance(mark, Mapping) or set(mark) != set(SWAP_INSTRUMENTS):
        raise C5AError("C5A mark dataset set mismatch")
    ...
    for instrument in SPOT_INSTRUMENTS:
        ...
        if current != timestamps:
            raise C5AError(f"spot alignment mismatch: {instrument}")
        if (
            ...
            raise C5AError(f"non-positive price or negative volume: {instrument}")
        invalid = (
        ...
        if np.any(invalid):
            raise C5AError(f"invalid spot OHLC geometry: {instrument}")
    ...
    for instrument in SWAP_INSTRUMENTS:
        ...
        if current != timestamps:
            raise C5AError(f"swap alignment mismatch: {instrument}")
        if np.any(arrays["quote_volume"] < 0):
            raise C5AError(f"negative swap quote volume: {instrument}")
        ...
        if current != timestamps:
            raise C5AError(f"mark alignment mismatch: {instrument}")
        if np.any(arrays["close"] <= 0):
            raise C5AError(f"non-positive mark close: {instrument}")
        mark_close[instrument] = arrays["close"]

    if timestamps is None:
        raise C5AError("empty C5A market")
    return C5AMarket(
...
def _index_of(market: C5AMarket, timestamp: datetime) -> int:
    try:
        ...
        raise C5AError(f"timestamp absent from market: {timestamp.isoformat()}") from exc
...
def calibration_decision_times(config: Mapping[str, Any]) -> tuple[datetime, ...]:
    ...
    while current <= end:
        if current.weekday() != 0 or current.hour != 0:
            raise C5AError("calibration boundary is not Monday 00 UTC")
        values.append(current)
        current += timedelta(days=7)
    if len(values) != 39:
        raise C5AError(f"calibration observation count mismatch: {len(values)}")
    return tuple(values)
...
def window_decision_times(window: Mapping[str, Any]) -> tuple[datetime, ...]:
    ...
    while current < end:
        if current.weekday() != 0 or current.hour != 0:
            raise C5AError("screen boundary is not Monday 00 UTC")
        values.append(current)
        current += timedelta(days=7)
    if len(values) != 13:
        raise C5AError(f"half-window decision count mismatch: {len(values)}")
    return tuple(values)
...
def _raw_features(
    ...
    *,
    ...
    if signal_index < 0:
        raise C5AError("missing completed signal bar")
    signal_time = market.timestamps[signal_index]
    if signal_time.weekday() != 6 or signal_time.hour != 20:
        raise C5AError("C5A signal must use Sunday 20 UTC")
    ...
    if signal_index - trend_intervals < 0 or signal_index - vol_count < 0:
        raise C5AError("insufficient C5A warm-up")
    ...
    for spot in SPOT_INSTRUMENTS:
        ...
        if not math.isfinite(basis) or not math.isfinite(participation) or participation < 0:
            raise C5AError(f"invalid crowding feature: {spot}")
        output[spot] = {
            "trend_28d": trend,
            "rv_28d": volatility,
            "basis_7d": basis,
...
def build_calibration(
    ...
    if len(rows) != 117:
        raise C5AError("calibration row count mismatch")
    for spot in SPOT_INSTRUMENTS:
        if any(len(observations[spot][field]) != 39 for field in observations[spot]):
            raise C5AError("calibration vector count mismatch")
    hashes = {
        spot: {
...
def _ecdf_percentile(values: Sequence[float], x: float) -> float:
    if len(values) != 39:
        raise C5AError("ECDF calibration count mismatch")
