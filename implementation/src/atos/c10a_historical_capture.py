"""Official-public, two-phase OKX custody for the frozen C10A screen."""
...
class C10AHistoricalCaptureError(RuntimeError):
    """Raised unless C10A public custody is exact, complete, and immutable."""
...
def _timestamp_ms(value: Any) -> int:
    try:
        ...
        raise C10AHistoricalCaptureError(f"invalid timestamp: {value!r}") from exc
    if stamp.tzinfo is None:
        raise C10AHistoricalCaptureError("timestamp must be timezone-aware")
...
def _pause(value: float) -> float:
    if isinstance(value, bool):
        raise C10AHistoricalCaptureError("request pause must be numeric")
    try:
        result = float(value)
    except (TypeError, ValueError) as exc:
        raise C10AHistoricalCaptureError("request pause must be numeric") from exc
    if not math.isfinite(result) or not 0 <= result <= 5:
        raise C10AHistoricalCaptureError(
            "request pause must be between zero and five seconds"
        )
    return result


def validate_c10a_public_request(request: PublicRequest) -> None:
    validate_public_request(
        request,
...
def _retain(
    ...
    if (
        ...
        raise C10AHistoricalCaptureError("capture transport provenance mismatch")
    try:
        package.retain_raw(raw, record)
    except C7AHistoricalCaptureError as exc:
        raise C10AHistoricalCaptureError(str(exc)) from exc
...
def _select_exact_candles(
    ...
    *,
    ...
    if start >= end or (end - start) % HOUR_MS:
        raise C10AHistoricalCaptureError("candle interval must be whole positive hours")
    by_time: dict[int, Mapping[str, Any]] = {}
    for row in rows:
        if row.get("instrument") != instrument:
            raise C10AHistoricalCaptureError("candle instrument mismatch")
        stamp = _timestamp_ms(row.get("timestamp"))
        if stamp in by_time:
            raise C10AHistoricalCaptureError("duplicate candle timestamp")
        by_time[stamp] = row
...
def _capture_candle_range(
    ...
    *,
    ...
    if instrument not in CANDIDATE_POOL or type(max_pages) is not int or not (
        ...
        raise C10AHistoricalCaptureError("invalid candle capture policy")
    if series_type not in {"formation_trades", "trades", "marks"}:
        raise C10AHistoricalCaptureError("invalid candle series type")
    pause = _pause(page_pause_seconds)
    start = _timestamp_ms(start_inclusive)
    cursor = _timestamp_ms(end_exclusive)
    if start >= cursor:
        raise C10AHistoricalCaptureError("candle capture interval must be positive")
    ...
    for page in range(max_pages):
        ...
        try:
            ...
            raise C10AHistoricalCaptureError(str(exc)) from exc
        if not rows:
            raise C10AHistoricalCaptureError("candle pagination returned an empty page")
        ...
        raise C10AHistoricalCaptureError("candle pagination exceeded max_pages")
...
def capture_trade_range(
    ...
    *,
    ...
    start_inclusive: str,
    end_exclusive: str,
    fetch_page: PageFetcher = fetch_raw_c10a,
    ...
    if series_type not in {"formation_trades", "trades"}:
        raise C10AHistoricalCaptureError("trade series must be formation_trades or trades")
...
def select_formation_universe(
    ...
    if set(formation) != set(CANDIDATE_POOL):
        raise C10AHistoricalCaptureError("formation candidate-pool coverage drift")
    ...
    if len(selected) != SELECTED_UNIVERSE_SIZE:
        raise C10AHistoricalCaptureError("selected universe size drift")
    return {
        "schema_version": 1,
        "stage": "C10A_FORMATION_UNIVERSE",
...
class C10AFundingDownloadSpec:
    ...
    def request(self) -> PublicRequest:
        if self.instrument not in CANDIDATE_POOL:
            raise C10AHistoricalCaptureError("funding instrument is not in C10A pool")
        try:
...
def capture_funding_downloads(
    ...
    *,
    ...
    specs: Sequence[C10AFundingDownloadSpec],
    start_inclusive: str,
    end_exclusive: str,
    fetch_object: PageFetcher = fetch_raw_c10a,
    ...
    if selected != package.selected_universe or not specs or {
        ...
        raise C10AHistoricalCaptureError("funding inventory does not match frozen top eight")
    request_ids = [spec.request_id for spec in specs]
    if len(request_ids) != len(set(request_ids)):
        raise C10AHistoricalCaptureError("funding request IDs must be unique")
...
def capture_historical_funding_range(
    ...
    *,
    ...
    start_inclusive: str,
    end_exclusive: str,
    fetch_manifest: PageFetcher = fetch_raw_c10a,
    fetch_object: PageFetcher = fetch_raw_c10a,
    ...
    if selected != package.selected_universe:
        raise C10AHistoricalCaptureError("funding selection is not the frozen top eight")
    pause = _pause(request_pause_seconds)
    specs: list[C10AFundingDownloadSpec] = []
    request_count = 0
    for instrument in selected:
        try:
...
class C10ACapturePackage(CapturePackage):
    """Immutable two-phase C10A source package over explicit public policy."""
    ...
    def freeze_universe(self, evidence: Mapping[str, Any]) -> tuple[str, ...]:
        ...
        if self._selected_universe:
            raise C10AHistoricalCaptureError("formation universe is already frozen")
        selected = evidence.get("selected_universe")
        scores = evidence.get("scores")
        if (
            evidence.get("schema_version") != 1
            or evidence.get("stage") != "C10A_FORMATION_UNIVERSE"
            ...
            raise C10AHistoricalCaptureError("invalid formation selected universe")
        ...
        for rank, row in enumerate(scores, start=1):
            if (
                ...
                raise C10AHistoricalCaptureError("invalid formation score evidence")
            ...
            if not value.is_finite() or value < 0:
                raise C10AHistoricalCaptureError("formation score must be non-negative")
    ...
    def retain_c10a_series(
        ...
        *,
        ...
        if not allowed:
            raise C10AHistoricalCaptureError("series identity is outside frozen phase")
        if key in self._normalized_series or not rows:
            raise C10AHistoricalCaptureError("series is empty or already retained")
        if _timestamp_ms(start_inclusive) >= _timestamp_ms(end_exclusive):
            raise C10AHistoricalCaptureError("series interval must be positive")
    ...
    def finalize(
        ...
        if not EXACT_SHA.fullmatch(implementation_sha):
            raise C10AHistoricalCaptureError("implementation SHA must be exact")
        plan = dict(capture_plan_value)
        if plan != capture_plan():
            raise C10AHistoricalCaptureError("capture plan is not frozen C10A H1-H5")
        if not self.records or len(self._selected_universe) != SELECTED_UNIVERSE_SIZE:
            raise C10AHistoricalCaptureError("capture records or selected universe missing")
        expected = {
        ...
        if any(path.is_symlink() for path in entries):
            raise C10AHistoricalCaptureError("capture package contains a symbolic link")
        files = []
        ...
        manifest = {
            "schema_version": 1,
            "stage": "C10A_HISTORICAL_CAPTURE_PACKAGE",
            "implementation_sha": implementation_sha,
