"""Deterministic C14A liquidity signal, continuous-notional ledger, and replay."""
...
class C14AHistoricalReplayError(RuntimeError):
...
def _time(value: Any) -> datetime:
    try:
        ...
        raise C14AHistoricalReplayError(f"invalid timestamp: {value!r}") from exc
    if stamp.tzinfo is None:
        raise C14AHistoricalReplayError("timestamp must be timezone-aware")
    return stamp.astimezone(UTC)


def _decimal(value: Any, label: str, *, positive: bool = False) -> Decimal:
    if isinstance(value, bool):
        raise C14AHistoricalReplayError(f"{label} must be decimal")
    try:
        result = Decimal(str(value))
    except (InvalidOperation, ValueError) as exc:
        raise C14AHistoricalReplayError(f"{label} must be decimal") from exc
    if not result.is_finite() or (positive and result <= 0):
        qualifier = "positive finite" if positive else "finite"
        raise C14AHistoricalReplayError(f"{label} must be {qualifier}")
    return result
...
def _hourly_prices(
    ...
    *,
    ...
    if set(rows_by_instrument) != set(selected) or len(selected) != len(set(selected)):
        raise C14AHistoricalReplayError(f"{label} instrument inventory drift")
    ...
    for instrument in selected:
        ...
        for row in rows:
            ...
            if stamp.minute or stamp.second or stamp.microsecond:
                raise C14AHistoricalReplayError(f"{label} timestamp is off-grid")
            if previous is not None and stamp <= previous:
                raise C14AHistoricalReplayError(f"{label} rows are duplicate or unordered")
            previous = stamp
        ...
        if not timestamps or (reference is not None and timestamps != reference):
            raise C14AHistoricalReplayError(f"{label} timestamps are empty or misaligned")
...
def _funding_rows(
    ...
    *,
    ...
    if set(rows_by_instrument) != set(selected):
        raise C14AHistoricalReplayError("funding instrument inventory drift")
    ...
    for instrument in selected:
        ...
        for row in rows_by_instrument[instrument]:
            ...
            if previous is not None and stamp <= previous:
                raise C14AHistoricalReplayError(
            ...
            if instrument in by_instrument:
                raise C14AHistoricalReplayError("duplicate funding settlement")
...
def build_signal(
    ...
    *,
    selected_universe: Sequence[str],
    trade_opens: Mapping[str, Mapping[datetime, Decimal]],
    trade_quote_volumes: Mapping[str, Mapping[datetime, Decimal]],
    policy: str = CANDIDATE_ID,
) -> dict[str, Any]:
    """Rank 28 complete UTC-day Amihud observations known at the cutoff."""
    ...
    if (
        ...
        or selected != CANDIDATE_POOL
        or set(trade_opens) != set(selected)
        or set(trade_quote_volumes) != set(selected)
    ):
        raise C14AHistoricalReplayError("signal policy or selected universe drift")
    ...
    if decision.weekday() != 0 or any(
        ...
        raise C14AHistoricalReplayError("signal decision is not Monday 00:00 UTC")
    cutoff = decision - SIGNAL_EXECUTION_LAG_HOURS * HOUR
    first_stamp = cutoff - SIGNAL_LOOKBACK_COMPLETE_DAYS * 24 * HOUR
    stamps = tuple(
        first_stamp + offset * 24 * HOUR
        for offset in range(SIGNAL_LOOKBACK_COMPLETE_DAYS + 1)
    )
    returns: dict[str, tuple[Decimal, ...]] = {}
    quote_volumes: dict[str, tuple[Decimal, ...]] = {}
    for instrument in selected:
        prices = trade_opens[instrument]
        hourly_volumes = trade_quote_volumes[instrument]
        try:
            points = [prices[stamp] for stamp in stamps]
            hourly_volume_days = tuple(
                tuple(
                    hourly_volumes[stamps[day] + hour * HOUR]
                    for hour in range(24)
                )
                for day in range(SIGNAL_LOOKBACK_COMPLETE_DAYS)
            )
        except KeyError as exc:
            raise C14AHistoricalReplayError(
                f"signal missing exact trade open or quote volume: {instrument} "
                f"{iso(exc.args[0])}"
            ) from exc
        if any(
            not value.is_finite() or value <= 0
            for day in hourly_volume_days
            for value in day
        ):
            raise C14AHistoricalReplayError(
                "signal hourly quote volume must be positive finite"
            )
        daily_volumes = tuple(
            sum(day, Decimal(0)) for day in hourly_volume_days
        )
        values = tuple(
            (right / left).ln() for left, right in pairwise(points)
        )
        if (
            len(values) != SIGNAL_LOOKBACK_COMPLETE_DAYS
            or len(daily_volumes) != SIGNAL_LOOKBACK_COMPLETE_DAYS
            or not all(value.is_finite() for value in values)
            or not all(value.is_finite() and value > 0 for value in daily_volumes)
        ):
            raise C14AHistoricalReplayError("signal return vector is invalid")
        returns[instrument] = values
        quote_volumes[instrument] = daily_volumes

    rows: list[dict[str, Any]] = []
    for instrument in selected:
        daily = returns[instrument]
        volumes = quote_volumes[instrument]
        mean_absolute_return = sum((abs(value) for value in daily), Decimal(0)) / Decimal(
            SIGNAL_LOOKBACK_COMPLETE_DAYS
        )
        mean_quote_volume = sum(volumes, Decimal(0)) / Decimal(
            SIGNAL_LOOKBACK_COMPLETE_DAYS
        )
        inverse_quote_volume = Decimal(1) / mean_quote_volume
        amihud_illiquidity = sum(
            (abs(value) / volume for value, volume in zip(daily, volumes, strict=True)),
            Decimal(0),
        ) / Decimal(SIGNAL_LOOKBACK_COMPLETE_DAYS)
        score = {
            CANDIDATE_ID: amihud_illiquidity,
            "MeanAbsoluteReturnRankComparator": mean_absolute_return,
            "InverseQuoteVolumeRankComparator": inverse_quote_volume,
        }[policy]
        if not all(
            value.is_finite()
            for value in (*daily, *volumes, mean_absolute_return, mean_quote_volume, score)
        ):
            raise C14AHistoricalReplayError("signal statistic is non-finite")
        rows.append(
            {
                "instrument": instrument,
                "daily_log_returns": [format(value, "f") for value in daily],
                "daily_quote_volumes": [format(value, "f") for value in volumes],
                "daily_amihud": [
                    format(abs(value) / volume, "f")
                    for value, volume in zip(daily, volumes, strict=True)
                ],
                "mean_absolute_return": format(mean_absolute_return, "f"),
                "mean_quote_volume": format(mean_quote_volume, "f"),
                "inverse_quote_volume": format(inverse_quote_volume, "f"),
                "amihud_illiquidity": format(amihud_illiquidity, "f"),
                "ranking_score": format(score, "f"),
            }
        )
    rows.sort(
        key=lambda row: (-Decimal(str(row["ranking_score"])), str(row["instrument"]))
    )
    ...
    if set(longs) & set(shorts) or len(longs) != LONG_COUNT or len(shorts) != SHORT_COUNT:
        raise C14AHistoricalReplayError("signal long/short rank construction failed")
    directions = {
    ...
    return {
        ...
        "signal_cutoff": iso(cutoff),
        "first_signal_open": iso(first_stamp),
        "daily_return_count": SIGNAL_LOOKBACK_COMPLETE_DAYS,
        "daily_quote_volume_count": SIGNAL_LOOKBACK_COMPLETE_DAYS,
        "hourly_quote_volume_count": SIGNAL_LOOKBACK_COMPLETE_DAYS * 24,
        "rows": rows,
        "longs": list(longs),
        "shorts": list(shorts),
...
def _maximum_drawdown(values: Sequence[Decimal]) -> Decimal:
    if not values or any(not value.is_finite() or value <= 0 for value in values):
        raise C14AHistoricalReplayError("drawdown path is empty or invalid")
    peak = values[0]
    maximum = Decimal(0)
    for value in values:
        peak = max(peak, value)
...
def evaluate_historical_window(
    ...
    *,
    ...
    if selected_window not in HISTORICAL_WINDOWS:
        raise C14AHistoricalReplayError("window is not frozen C14A history")
    selected = tuple(selected_universe)
    if selected != CANDIDATE_POOL:
        raise C14AHistoricalReplayError("selected universe must equal fixed instruments")
    if cost_label not in COST_RATES or policy not in POLICIES:
        raise C14AHistoricalReplayError("unknown C14A policy or cost")
    ...
    trade_quote_volumes = _hourly_prices(
        trade_rows,
        selected_universe=selected,
        field="volume_quote",
        label="trade quote volume",
    )
    ...
    _require_exact_hours(
        ...
        - (SIGNAL_EXECUTION_LAG_HOURS + SIGNAL_LOOKBACK_COMPLETE_DAYS * 24)
        * HOUR,
        end_exclusive=selected_window.end_exclusive + HOUR,
        label="trade",
    )
    _require_exact_hours(
        next(iter(trade_quote_volumes.values())),
        start=selected_window.start
        - (SIGNAL_EXECUTION_LAG_HOURS + SIGNAL_LOOKBACK_COMPLETE_DAYS * 24)
        * HOUR,
        end_exclusive=selected_window.end_exclusive + HOUR,
    ...
    _require_exact_hours(
        ...
        start=selected_window.start - HOUR,
    ...
    while current < selected_window.end_exclusive:
        ...
        if is_decision:
            if weekly_start is not None:
                ...
                if weekly_start_time is None:
                    raise C14AHistoricalReplayError("weekly start time is missing")
        ...
        if pending_forced_close:
            ...
            if equity <= 0:
                raise C14AHistoricalReplayError("non-positive equity before forced close")
        ...
        if is_decision and not suppress_decision:
            ...
            if pretrade_equity <= 0 or equity <= 0:
                raise C14AHistoricalReplayError("non-positive pretrade equity")
            pretrade_equity = equity
            signal = build_signal(
                current,
                selected_universe=selected,
                trade_opens=trades,
                trade_quote_volumes=trade_quote_volumes,
                policy=policy,
            )
            ...
            if abs(signed_target) > RECONCILIATION_TOLERANCE:
                raise C14AHistoricalReplayError("dollar-neutral target drift")
            expected_gross = GROSS_NOTIONAL * pretrade_equity
            if abs(gross_target - expected_gross) > RECONCILIATION_TOLERANCE:
                raise C14AHistoricalReplayError("gross target drift")
            turnover_sum += changed_notional
        ...
        if not equity.is_finite() or equity <= 0:
            raise C14AHistoricalReplayError("hourly equity is non-positive or non-finite")
        observe_buffer()
        record_event(current + HOUR, "HOURLY_MARK")
        current += HOUR

    if weekly_start is None or weekly_start_time is None:
        raise C14AHistoricalReplayError("window produced no weekly start")
    changed_notional = Decimal(0)
    for instrument in selected:
    ...
    if equity <= 0:
        raise C14AHistoricalReplayError("non-positive terminal pretrade equity")
    for instrument in selected:
        boundary_open = trades[instrument][selected_window.end_exclusive]
        delta = -quantities[instrument]
    ...
    if abs(residual) > RECONCILIATION_TOLERANCE or any(quantities.values()):
        raise C14AHistoricalReplayError("window accounting reconciliation failure")
    return {
        "stage": "C14A_HISTORICAL_WINDOW",
...
def _btc_weekly_mark_returns(
    ...
    for start in decision_times(window):
        ...
        if not value.is_finite():
            raise C14AHistoricalReplayError("BTC weekly benchmark return is invalid")
...
def evaluate_historical_window_matrix(
    ...
    *,
    ...
    if (
        ...
        raise C14AHistoricalReplayError("window matrix source inventory drift")
    ...
    return {
        "schema_version": 1,
        "stage": "C14A_HISTORICAL_WINDOW_MATRIX",
        "window": selected_window.to_dict(),
...
def _statistics(values: Sequence[Decimal]) -> dict[str, Any]:
    ...
    if len(raw) != EXPECTED_TOTAL_DECISIONS or not all(
        ...
        raise C14AHistoricalReplayError("pooled weekly return coverage is invalid")
...
def _pool_policy(
    ...
    weekly_equities = [
        Decimal(str(bucket["start_equity"]))
        for row in ordered
        for bucket in row["weekly_buckets"]
    ]
    if (
        len(weekly_equities) != EXPECTED_TOTAL_DECISIONS
        or any(value <= 0 or not value.is_finite() for value in weekly_equities)
    ):
        raise C14AHistoricalReplayError("mean weekly equity coverage is invalid")
    mean_weekly_equity = sum(weekly_equities, Decimal(0)) / Decimal(
        len(weekly_equities)
    )
    total_absolute_changed_notional = sum(
        (Decimal(str(row["turnover_sum"])) for row in ordered), Decimal(0)
    )
    annualized_one_way_turnover = (
        Decimal("0.5")
        * total_absolute_changed_notional
        / mean_weekly_equity
        * Decimal(52)
        / Decimal(len(weekly_equities))
    )
    ...
    return {
        ...
        "mean_weekly_equity": format(mean_weekly_equity, "f"),
        "total_absolute_changed_notional": format(
            total_absolute_changed_notional, "f"
        ),
        "annualized_one_way_turnover": format(annualized_one_way_turnover, "f"),
...
def _btc_beta(candidate: Sequence[Decimal], benchmark: Sequence[Decimal]) -> Decimal:
    if len(candidate) != EXPECTED_TOTAL_DECISIONS or len(benchmark) != len(candidate):
        raise C14AHistoricalReplayError("BTC beta return coverage is invalid")
    ...
    if variance <= 0 or not covariance.is_finite() or not variance.is_finite():
        raise C14AHistoricalReplayError("BTC beta regression is invalid")
    result = covariance / variance
    if not result.is_finite():
        raise C14AHistoricalReplayError("BTC beta is non-finite")
    return result


def summarize_h1_h5(windows: Mapping[str, Any]) -> dict[str, Any]:
    """Pool independent capital and apply every preregistered C14A gate."""

    expected_ids = {window.window_id for window in HISTORICAL_WINDOWS}
    if set(windows) != expected_ids:
        raise C14AHistoricalReplayError("C14A summary requires exactly H1-H5")
    selected_universes = {
        tuple(windows[window.window_id].get("selected_universe", ()))
        for window in HISTORICAL_WINDOWS
    }
    if selected_universes != {CANDIDATE_POOL}:
        raise C14AHistoricalReplayError("fixed universe drift across H1-H5")
    pooled = {
        policy: {
    ...
    mean_absolute_return = pooled["MeanAbsoluteReturnRankComparator"]["1.0x"]
    inverse_quote_volume = pooled["InverseQuoteVolumeRankComparator"]["1.0x"]
    ...
    mean_absolute_return_sharpe = Decimal(
        str(mean_absolute_return["statistics"]["annualized_weekly_sharpe"])
    )
    inverse_quote_volume_sharpe = Decimal(
        str(inverse_quote_volume["statistics"]["annualized_weekly_sharpe"])
    )
    gates = {
        ...
        <= Decimal("0.15"),
        "absolute_btc_beta": abs(btc_beta) <= Decimal("0.15"),
        ...
        "return_delta_vs_mean_absolute_return": Decimal(candidate["aggregate_return"])
        - Decimal(mean_absolute_return["aggregate_return"])
        > 0,
        "sharpe_delta_vs_mean_absolute_return": (
            candidate_sharpe - mean_absolute_return_sharpe
        )
        >= Decimal("0.10"),
        "drawdown_no_worse_than_mean_absolute_return": Decimal(
            candidate["maximum_drawdown"]
        )
        <= Decimal(mean_absolute_return["maximum_drawdown"]),
        "turnover_no_greater_than_mean_absolute_return": Decimal(
            candidate["annualized_one_way_turnover"]
        )
        <= Decimal(mean_absolute_return["annualized_one_way_turnover"]),
        "return_delta_vs_inverse_quote_volume": Decimal(candidate["aggregate_return"])
        - Decimal(inverse_quote_volume["aggregate_return"])
        > 0,
        "sharpe_delta_vs_inverse_quote_volume": (
            candidate_sharpe - inverse_quote_volume_sharpe
        )
        >= Decimal("0.10"),
        "drawdown_no_worse_than_inverse_quote_volume": Decimal(
            candidate["maximum_drawdown"]
        )
        <= Decimal(inverse_quote_volume["maximum_drawdown"]),
        "turnover_no_greater_than_inverse_quote_volume": Decimal(
            candidate["annualized_one_way_turnover"]
        )
        <= Decimal(inverse_quote_volume["annualized_one_way_turnover"]),
    }
    selected = all(gates.values())
    return {
        "schema_version": 1,
        "stage": "C14A_H1_H5_POOLED_SUMMARY",
