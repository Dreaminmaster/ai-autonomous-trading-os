"""Exact clean-checkout binding for the one-shot C10A authority run."""
...
class C10AHistoricalRunGuardError(RuntimeError):
...
def validate_checkout_binding(
    ...
    if not EXACT_SHA.fullmatch(implementation_sha) or dict(value) != expected:
        raise C10AHistoricalRunGuardError(
            "C10A checkout binding is dirty or SHA-mismatched"
        )
    return expected
...
def verify_checkout_binding(
    ...
    *,
    ...
    if not EXACT_SHA.fullmatch(implementation_sha):
        raise C10AHistoricalRunGuardError("implementation SHA must be exact")
    ...
    try:
        ...
        raise C10AHistoricalRunGuardError(
            "unable to verify the C10A Git checkout"
        ) from exc
    if head != implementation_sha or status:
        raise C10AHistoricalRunGuardError(
            "C10A authority requires its exact clean implementation checkout"
        )
