...
<body><main>
  ...
  <section class="lower"><div class="card"><h2>历史 Segments</h2><div style="overflow:auto"><table><thead><tr><th>#</th><th>状态</th><th>开始</th><th>结束/heartbeat</th><th>有效时间</th><th>Cycles</th><th>Fills</th><th>Failures</th><th>触发原因</th></tr></thead><tbody id="segments"></tbody></table></div></div><div class="card"><h2>最终验证条件</h2><div class="checks" id="checks"></div></div></section>
...
$('segments').innerHTML=(x.segments||[]).map(s=>`<tr><td>${s.sequence}</td><td>${esc(s.state)}</td><td>${when(s.started_at)}</td><td>${when(s.ended_at||s.last_valid_heartbeat)}</td><td>${dur(s.valid_duration_seconds)}</td><td>${s.valid_cycles||0}</td><td>${s.simulated_fills||0}</td><td>${s.failures||0}</td><td class="reason">${esc(failureText(s.last_failure))}</td></tr>`).join('')||'<tr><td colspan="9" class="muted">尚无 Segment</td></tr>';$('checks').innerHTML=(x.validation_conditions||[]).map(c=>`<div class="check"><span>${esc(c.name)}</span><b class="${c.passed?'ok':'bad'}">${c.passed?'✓':'✕'}</b></div>`).join('')||'<div class="muted">创建 Campaign 后显示</div>';
