"""Frozen C13A cross-sectional lottery-demand reversal research contract."""
...
CANDIDATE_ID = "C13ACrossSectionalLotteryDemandReversal"
BTC_BETA_BENCHMARK = "BTC-USDT-SWAP"

HOUR = timedelta(hours=1)
WEEK = timedelta(days=7)
TRADE_CAPTURE_START = datetime(2023, 12, 24, tzinfo=UTC)
...
class C13AContractError(RuntimeError):
    """Raised when a frozen C13A identity or time boundary drifts."""


def iso(value: datetime) -> str:
    if value.tzinfo is None:
        raise C13AContractError("C13A timestamp must be timezone-aware")
...
def window_by_id(window_id: str) -> HistoricalWindow:
    ...
    if len(matches) != 1:
        raise C13AContractError(f"unknown C13A window: {window_id!r}")
    return matches[0]
...
def decision_times(window: HistoricalWindow | str) -> tuple[datetime, ...]:
    ...
    while current < selected.end_exclusive:
        if current.weekday() != 0 or any(
            ...
            raise C13AContractError("C13A decision must be Monday 00:00 UTC")
        output.append(current)
        current += WEEK
    if len(output) != EXPECTED_DECISIONS_PER_WINDOW or current != selected.end_exclusive:
        raise C13AContractError("each C13A window must contain exactly 26 weeks")
    return tuple(output)
...
def validate_contract() -> dict[str, Any]:
    ...
    if report.get("familywise_trial_count") != EXPECTED_FAMILYWISE_TRIALS:
        raise C13AContractError("C13A program trial-count drift")
    previous = None
    decisions = 0
