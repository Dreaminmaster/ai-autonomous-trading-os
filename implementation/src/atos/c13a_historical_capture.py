"""Official-public, two-phase OKX custody for the frozen C13A screen."""
...
class C13AHistoricalCaptureError(RuntimeError):
    """Raised unless C13A public custody is exact, complete, and immutable."""
...
def _timestamp_ms(value: Any) -> int:
    try:
        ...
        raise C13AHistoricalCaptureError(f"invalid timestamp: {value!r}") from exc
    if stamp.tzinfo is None:
        raise C13AHistoricalCaptureError("timestamp must be timezone-aware")
...
def _pause(value: float) -> float:
    if isinstance(value, bool):
        raise C13AHistoricalCaptureError("request pause must be numeric")
    try:
        result = float(value)
    except (TypeError, ValueError) as exc:
        raise C13AHistoricalCaptureError("request pause must be numeric") from exc
    if not math.isfinite(result) or not 0 <= result <= 5:
        raise C13AHistoricalCaptureError(
            "request pause must be between zero and five seconds"
        )
    return result


def validate_c13a_public_request(request: PublicRequest) -> None:
    validate_public_request(
        request,
...
def _retain(
    ...
    if (
        ...
        raise C13AHistoricalCaptureError("capture transport provenance mismatch")
    try:
        package.retain_raw(raw, record)
    except C7AHistoricalCaptureError as exc:
        raise C13AHistoricalCaptureError(str(exc)) from exc
...
def _select_exact_candles(
    ...
    *,
    ...
    if start >= end or (end - start) % HOUR_MS:
        raise C13AHistoricalCaptureError("candle interval must be whole positive hours")
    by_time: dict[int, Mapping[str, Any]] = {}
    for row in rows:
        if row.get("instrument") != instrument:
            raise C13AHistoricalCaptureError("candle instrument mismatch")
        stamp = _timestamp_ms(row.get("timestamp"))
        if stamp in by_time:
            raise C13AHistoricalCaptureError("duplicate candle timestamp")
        by_time[stamp] = row
...
def _capture_candle_range(
    ...
    *,
    ...
    if instrument not in CANDIDATE_POOL or type(max_pages) is not int or not (
        ...
        raise C13AHistoricalCaptureError("invalid candle capture policy")
    if series_type not in {"trades", "marks"}:
        raise C13AHistoricalCaptureError("invalid candle series type")
    pause = _pause(page_pause_seconds)
    start = _timestamp_ms(start_inclusive)
    cursor = _timestamp_ms(end_exclusive)
    if start >= cursor:
        raise C13AHistoricalCaptureError("candle capture interval must be positive")
    ...
    for page in range(max_pages):
        ...
        try:
            ...
            raise C13AHistoricalCaptureError(str(exc)) from exc
        if not rows:
            raise C13AHistoricalCaptureError("candle pagination returned an empty page")
        ...
        raise C13AHistoricalCaptureError("candle pagination exceeded max_pages")
...
def capture_trade_range(
    ...
    *,
    ...
    start_inclusive: str,
    end_exclusive: str,
    fetch_page: PageFetcher = fetch_raw_c13a,
    ...
    if series_type != "trades":
        raise C13AHistoricalCaptureError("trade series must be frozen C13A trades")
...
class C13AFundingDownloadSpec:
    ...
    def request(self) -> PublicRequest:
        if self.instrument not in CANDIDATE_POOL:
            raise C13AHistoricalCaptureError("funding instrument is not in C13A pool")
        try:
...
def capture_funding_downloads(
    ...
    *,
    ...
    specs: Sequence[C13AFundingDownloadSpec],
    start_inclusive: str,
    end_exclusive: str,
    fetch_object: PageFetcher = fetch_raw_c13a,
    ...
    if selected != package.selected_universe or not specs or {
        ...
        raise C13AHistoricalCaptureError("funding inventory does not match fixed universe")
    request_ids = [spec.request_id for spec in specs]
    if len(request_ids) != len(set(request_ids)):
        raise C13AHistoricalCaptureError("funding request IDs must be unique")
...
def capture_historical_funding_range(
    ...
    *,
    ...
    start_inclusive: str,
    end_exclusive: str,
    fetch_manifest: PageFetcher = fetch_raw_c13a,
    fetch_object: PageFetcher = fetch_raw_c13a,
    ...
    if selected != package.selected_universe:
        raise C13AHistoricalCaptureError("funding selection is not the fixed universe")
    pause = _pause(request_pause_seconds)
    specs: list[C13AFundingDownloadSpec] = []
    request_count = 0
    for instrument in selected:
        try:
...
class C13ACapturePackage(CapturePackage):
    """Immutable fixed-universe C13A package over official public sources."""
    ...
    def retain_c13a_series(
        ...
        *,
        ...
        if not allowed:
            raise C13AHistoricalCaptureError("series identity is outside frozen phase")
        if key in self._normalized_series or not rows:
            raise C13AHistoricalCaptureError("series is empty or already retained")
        if _timestamp_ms(start_inclusive) >= _timestamp_ms(end_exclusive):
            raise C13AHistoricalCaptureError("series interval must be positive")
    ...
    def finalize(
        ...
        if not EXACT_SHA.fullmatch(implementation_sha):
            raise C13AHistoricalCaptureError("implementation SHA must be exact")
        plan = dict(capture_plan_value)
        if plan != capture_plan():
            raise C13AHistoricalCaptureError("capture plan is not frozen C13A H1-H5")
        if not self.records or self._selected_universe != CANDIDATE_POOL:
            raise C13AHistoricalCaptureError("capture records or fixed universe missing")
        ...
        if any(path.is_symlink() for path in entries):
            raise C13AHistoricalCaptureError("capture package contains a symbolic link")
        files = []
        ...
        manifest = {
            "schema_version": 1,
            "stage": "C13A_HISTORICAL_CAPTURE_PACKAGE",
            "implementation_sha": implementation_sha,
