"""Immutable C11A H1-H5 public-data and economic evidence package."""
...
CONFIG_PATH = Path(
    "implementation/config/c11a_cross_sectional_idiosyncratic_volatility.json"
)
REGISTRY_PATH = Path("implementation/config/phase_c_research_program_registry_v2.json")
...
SHARED_RUNTIME_PATHS = (
    ...
    Path("implementation/src/atos/c11a_research_program_guard.py"),
)
API_FAMILIES = {
...
class C11AHistoricalEvidenceError(RuntimeError):
    """Raised when final evidence cannot be proven complete and immutable."""


class C11AHistoricalDataEvidenceError(C11AHistoricalEvidenceError):
...
def _read_json(path: Path) -> Any:
    try:
        ...
        raise C11AHistoricalEvidenceError(f"invalid retained JSON: {path}") from exc
...
class C11AEvidencePackage:
    """Exclusive, atomic, no-overwrite evidence writer."""

    def __init__(self, root: Path):
        self.root = Path(root)
        if self.root.exists():
            raise C11AHistoricalEvidenceError(
    ...
    def write_json(self, relative: str, value: Any) -> None:
        if self._finalized:
            raise C11AHistoricalEvidenceError("evidence package is finalized")
        path = Path(relative)
        if path.is_absolute() or not path.parts or ".." in path.parts:
            raise C11AHistoricalEvidenceError("evidence path escapes package")
    ...
    def finalize(self, *, implementation_sha: str) -> dict[str, Any]:
        if not EXACT_SHA.fullmatch(implementation_sha):
            raise C11AHistoricalEvidenceError("implementation SHA must be exact")
        entries = sorted(self.root.rglob("*"))
        if any(path.is_symlink() for path in entries):
            raise C11AHistoricalEvidenceError("evidence contains a symbolic link")
        files = []
        ...
        if not files:
            raise C11AHistoricalEvidenceError("evidence package contains no files")
        manifest = {
            "schema_version": 1,
            "stage": "C11A_H1_H5_HISTORICAL_EVIDENCE_PACKAGE",
...
def verify_capture_package(
    ...
    except C11AHistoricalEvidenceError as exc:
        raise C11AHistoricalDataEvidenceError(str(exc)) from exc
...
def verify_evidence_package(
    ...
    if root.is_symlink() or any(path.is_symlink() for path in root.rglob("*")):
        raise C11AHistoricalEvidenceError("evidence package contains a symbolic link")
    manifest = _read_json(root / "manifest.json")
    if not isinstance(manifest, Mapping) or (
        manifest.get("stage") != "C11A_H1_H5_HISTORICAL_EVIDENCE_PACKAGE"
        or manifest.get("implementation_sha") != implementation_sha
        or any(
            manifest.get(key) != value for key, value in safety_boundary().items()
        )
    ):
        raise C11AHistoricalEvidenceError("evidence manifest identity drift")
    declared = manifest.get("files")
    if not isinstance(declared, list) or manifest.get("file_count") != len(declared):
        raise C11AHistoricalEvidenceError("evidence manifest inventory is invalid")
    observed = {
    ...
    if observed != declared_paths or len(declared_paths) != len(declared):
        raise C11AHistoricalEvidenceError("evidence manifest path inventory mismatch")
    for row in declared:
        if not isinstance(row, Mapping):
            raise C11AHistoricalEvidenceError("evidence manifest row is invalid")
...
def _verify_capture_package(root: Path, *, implementation_sha: str) -> dict[str, Any]:
    if root.is_symlink() or any(path.is_symlink() for path in root.rglob("*")):
        raise C11AHistoricalEvidenceError("capture package contains a symbolic link")
    manifest = _read_json(root / "manifest.json")
    index = _read_json(root / "capture_index.json")
    binding = _read_json(root / "checkout_binding.json")
    universe = _read_json(root / "normalized" / "formation_universe.json")
    if not all(isinstance(value, Mapping) for value in (manifest, index, universe)):
        raise C11AHistoricalEvidenceError("capture authority is not an object")
    if (
        manifest.get("stage") != "C11A_HISTORICAL_CAPTURE_PACKAGE"
        or index.get("stage") != "C11A_HISTORICAL_CAPTURE"
        or manifest.get("implementation_sha") != implementation_sha
        or index.get("implementation_sha") != implementation_sha
        or index.get("capture_plan") != capture_plan()
        or index.get("selected_universe") != universe.get("selected_universe")
        or manifest.get("real_public_data") is not True
        or manifest.get("economic_result") is not False
    ):
        raise C11AHistoricalEvidenceError("capture identity, plan, or universe drift")
    if any(
        manifest.get(key) != value or index.get(key) != value
        for key, value in safety_boundary().items()
    ):
        raise C11AHistoricalEvidenceError("capture safety boundary drift")
    try:
        validate_checkout_binding(binding, implementation_sha=implementation_sha)
    except C11AHistoricalRunGuardError as exc:
        raise C11AHistoricalEvidenceError(str(exc)) from exc
    declared = manifest.get("files")
    if not isinstance(declared, list) or manifest.get("file_count") != len(declared):
        raise C11AHistoricalEvidenceError("capture manifest inventory is invalid")
    observed = {
        path.relative_to(root).as_posix()
        for path in root.rglob("*")
        if path.is_file() and path.name != "manifest.json"
    ...
    if observed != paths or len(paths) != len(declared):
        raise C11AHistoricalEvidenceError("capture manifest path inventory mismatch")
    for row in declared:
        if not isinstance(row, Mapping):
            raise C11AHistoricalEvidenceError("capture manifest row is invalid")
    ...
    if not isinstance(records, list) or not records:
        raise C11AHistoricalEvidenceError("capture request records are missing")
    request_ids = set()
    raw_paths = set()
    for record in records:
        if not isinstance(record, Mapping) or set(record) != {
            ...
            raise C11AHistoricalEvidenceError("capture request record schema drift")
        ...
        if (
            ...
            raise C11AHistoricalEvidenceError("capture request or raw path is duplicate")
        ...
        if relative != expected_relative or relative not in observed:
            raise C11AHistoricalEvidenceError("capture raw path identity drift")
        ...
        if (
            ...
            raise C11AHistoricalEvidenceError("capture record/raw digest mismatch")
        ...
        if (
            ...
            raise C11AHistoricalEvidenceError("capture retry provenance drift")
        try:
        ...
        if (
            ...
            raise C11AHistoricalEvidenceError("capture time or media type is invalid")
        ...
        try:
            validate_c11a_public_request(requested)
            validate_c11a_public_request(final)
        except C7APublicDataError as exc:
            raise C11AHistoricalEvidenceError(
                "capture request URL is outside public C11A policy"
            ) from exc
    ...
    if raw_paths != {path for path in observed if path.startswith("raw/")}:
        raise C11AHistoricalEvidenceError("capture raw record inventory mismatch")
    ...
    if (
        ...
        raise C11AHistoricalEvidenceError("capture selected universe is invalid")
    return {
...
def _rows(root: Path, series: str, instruments: tuple[str, ...]) -> dict[str, list[dict[str, Any]]]:
    ...
    for instrument in instruments:
        ...
        if (
            ...
            raise C11AHistoricalDataEvidenceError(
...
def _authority_hashes(root: Path, implementation_sha: str) -> dict[str, Any]:
    files = [
        path
        for path in (root / "implementation" / "src" / "atos").glob("c11a_*.py")
        if path.is_file()
    ]
    files.extend(
        path
        for path in (root / "implementation" / "scripts").glob("c11a_*.py")
        if path.is_file()
    )
    ...
    if any(not path.is_file() for path in files):
        raise C11AHistoricalEvidenceError("authority source inventory is incomplete")
    inventory = []
...
def build_h1_h5_evidence_package(
    ...
    *,
    ...
    if not EXACT_SHA.fullmatch(implementation_sha) or not SAFE_RUN_ID.fullmatch(
        ...
        raise C11AHistoricalEvidenceError("implementation SHA or run ID is invalid")
    try:
    ...
    if stamp.tzinfo is None:
        raise C11AHistoricalEvidenceError("evaluation time must be timezone-aware")
    ...
    formation = _rows(capture_root, "formation_trades", CANDIDATE_POOL)
    ...
    retained_universe = _read_json(
        capture_root / "normalized" / "formation_universe.json"
    )
    formation_review = review_formation_universe(retained_universe, formation)
    ...
    independent_pass = (
        formation_review["status"] == "PASS"
        and all(review["status"] == "PASS" for review in independent.values())
    ...
    final = {
        "schema_version": 1,
        "stage": "C11A_H1_H5_FINAL_CLASSIFICATION",
    ...
    package = C11AEvidencePackage(Path(output_root))
    package.write_json(
        "metadata.json",
        {
            "schema_version": 1,
            "stage": "C11A_H1_H5_HISTORICAL_ECONOMIC_RUN",
            "authoritative_run_id": authoritative_run_id,
            "implementation_sha": implementation_sha,
    ...
    package.write_json("formation_independent_review.json", formation_review)
    package.write_json(
        "normalized_coverage.json",
        {
            series: {instrument: len(values) for instrument, values in rows.items()}
            for series, rows in (
                ("formation_trades", formation),
                ("trades", trades),
                ("marks", marks),
                ("funding", funding),
            )
    ...
    if verification["verified_file_count"] != manifest["file_count"]:
        raise C11AHistoricalEvidenceError("post-finalization evidence audit drift")
