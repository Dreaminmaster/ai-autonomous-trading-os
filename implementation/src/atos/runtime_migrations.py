...
_MIGRATION_0004_SQL = """
CREATE UNIQUE INDEX uq_execution_intents_order_semantics
ON execution_intents(execution_intent_id, symbol, action);

CREATE UNIQUE INDEX uq_dispatch_attempts_order_owner
...
CREATE TABLE order_states (
    ...
    FOREIGN KEY (execution_intent_id, symbol, side)
        REFERENCES execution_intents(execution_intent_id, symbol, action)
        ON DELETE RESTRICT
);
CREATE UNIQUE INDEX uq_order_states_symbol_owner
ON order_states(venue, account_scope, order_id, symbol);
...
CREATE TABLE fill_states (
    ...
    FOREIGN KEY (venue, account_scope, order_id, symbol)
        REFERENCES order_states(venue, account_scope, order_id, symbol)
        ON DELETE RESTRICT
);
CREATE UNIQUE INDEX uq_fill_states_symbol_identity
ON fill_states(venue, account_scope, fill_id, symbol);
...
WHERE status = 'OPEN';
CREATE UNIQUE INDEX uq_position_states_account_symbol
ON position_states(position_id, venue, account_scope, symbol);
