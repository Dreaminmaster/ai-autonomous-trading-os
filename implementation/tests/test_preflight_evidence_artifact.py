...
def _fail(mode, files, expected_reason=None, **kw):
    rc, out, err = _run(mode, files, **kw)
    assert rc != 0, f"expected FAIL got rc=0\nSTDOUT: {out}\nSTDERR: {err}"
    if expected_reason is not None:
        combined = out + err
        assert expected_reason in combined, f"expected reason {expected_reason!r} in output\nSTDOUT: {out}\nSTDERR: {err}"
...
def test_atos_missing_manifest(): _fail("atos-tests", {"pytest.log": "x", "no_secret_scan.log": "x", "freqtrade_data/backtest_results/walk_forward_report.json": "x"}, expected_reason="manifest missing")
def test_atos_invalid_manifest(): m = _manifest(); m["schema_version"] = "bad"; _fail("atos-tests", {"evidence_manifest.json": m, "pytest.log": "x", "no_secret_scan.log": "x", "freqtrade_data/backtest_results/walk_forward_report.json": "x"}, expected_reason="manifest sv!=1")
def test_atos_manifest_non_dict(): _fail("atos-tests", {"evidence_manifest.json": "[]", "pytest.log": "x", "no_secret_scan.log": "x", "freqtrade_data/backtest_results/walk_forward_report.json": "x"}, expected_reason="manifest not dict")
def test_atos_wrong_run_id(): m = _manifest(); m["run_id"] = "wrong"; _fail("atos-tests", {"evidence_manifest.json": m, "pytest.log": "x", "no_secret_scan.log": "x", "freqtrade_data/backtest_results/walk_forward_report.json": "x"}, expected_reason="manifest run_id")
def test_atos_wrong_head_sha(): m = _manifest(); m["head_sha"] = "wrong"; _fail("atos-tests", {"evidence_manifest.json": m, "pytest.log": "x", "no_secret_scan.log": "x", "freqtrade_data/backtest_results/walk_forward_report.json": "x"}, expected_reason="manifest sha")
def test_atos_wrong_job(): m = _manifest("wrong"); _fail("atos-tests", {"evidence_manifest.json": m, "pytest.log": "x", "no_secret_scan.log": "x", "freqtrade_data/backtest_results/walk_forward_report.json": "x"}, expected_reason="manifest job")
def test_atos_malformed_json(): _fail("atos-tests", {"evidence_manifest.json": "{broken-json", "pytest.log": "x", "no_secret_scan.log": "x", "freqtrade_data/backtest_results/walk_forward_report.json": "x"}, expected_reason="manifest JSON")
...
def test_anomaly_rc_zero(): la = _la_pass(); la["final_status"] = "PASS_WITH_RC_ANOMALY"; la["freqtrade_returncode"] = 0; la["explicit_no_bias_evidence"] = True; _fail("freqtrade", {"evidence_manifest.json": _manifest("freqtrade"), "freqtrade_data/backtest_results/canonical_baseline_summary.json": {}, "freqtrade_data/backtest_results/canonical_baseline_la_lookahead_status.json": _la_pass(), "validation_reports/strategy_fix_round1.json": _r1("PASS_WITH_RC_ANOMALY"), "freqtrade_data/backtest_results/v1_la_lookahead_status.json": la}, expected_reason="ANOMALY contract")
