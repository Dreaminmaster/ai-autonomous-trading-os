...
from atos.c13a_historical_capture import (
    C13ACapturePackage,
...
def _complete_package(root: Path) -> C13ACapturePackage:
...
def test_contract_freezes_exact_universe_clocks_comparators_and_safety() -> None:
    ...
    plan = capture_plan()
    assert report["familywise_trial_count"] == 628
    ...
    assert plan["trade_start_inclusive"] == "2023-12-24T00:00:00Z"
...
def test_public_request_policy_rejects_credentials_private_paths_and_other_assets() -> None:
    ...
    validate_c13a_public_request(request)
...
def test_trade_capture_retains_raw_before_strict_exact_normalization(tmp_path: Path) -> None:
    package = C13ACapturePackage(tmp_path / "capture")
...
def test_trade_capture_rejects_unconfirmed_candle(tmp_path: Path) -> None:
    ...
    with pytest.raises(C13AHistoricalCaptureError):
        capture_trade_range(
            package,
            series_type="trades",
            instrument="BTC-USDT-SWAP",
...
def test_capture_package_has_fixed_universe_and_recursive_hash_manifest(tmp_path: Path) -> None:
    ...
    assert manifest["stage"] == "C13A_HISTORICAL_CAPTURE_PACKAGE"
...
def test_capture_package_fails_closed_on_missing_series_overwrite_and_escape(tmp_path: Path) -> None:
    incomplete = C13ACapturePackage(tmp_path / "incomplete")
    with pytest.raises(C13AHistoricalCaptureError, match="records or fixed universe"):
        incomplete.finalize(implementation_sha=SHA, capture_plan_value=capture_plan())
    package = C13ACapturePackage(tmp_path / "guarded")
...
def test_package_rejects_formation_series_and_nonfixed_instrument(tmp_path: Path) -> None:
    ...
    for series_type, instrument in (
        ...
        with pytest.raises(C13AHistoricalCaptureError, match="outside frozen phase"):
            package.retain_c13a_series(
