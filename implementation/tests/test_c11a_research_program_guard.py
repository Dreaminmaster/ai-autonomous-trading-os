...
def test_repository_history_and_c11a_design_are_frozen() -> None:
    ...
    assert report["authority_count"] == 12
...
def test_registry_records_c10a_without_an_economic_trial() -> None:
    ...
    assert results["C10A"] == "PROGRAM_FAILURE_ECONOMICS_NOT_RUN"
    assert sum(counts.values()) == 626
    assert registry["prospective_stage"] == "C11A"
    assert registry["familywise_trial_count"] == 627


def test_bonferroni_boundary_is_exact_and_fail_closed() -> None:
    threshold = Decimal(1) - Decimal("0.05") / Decimal(627)
    ...
    with pytest.raises(C11AResearchProgramGuardError, match="trial-count drift"):
        bonferroni_adjusted_psr(threshold, trial_count=626)
    with pytest.raises(C11AResearchProgramGuardError, match=r"\[0, 1\]"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda payload: payload["stages"][-1].update({"result": "ECONOMIC_FAIL"}),
            "trial/result history",
        ),
        (
...
def test_registry_rejects_history_rewrite_or_promotion(
    ...
    mutation(registry)
    with pytest.raises(C11AResearchProgramGuardError, match=message):
...
def test_registry_hashes_all_authorities_and_rejects_tamper(tmp_path: Path) -> None:
    ...
    assert authorities[-1]["stage"] == "C10A"
    ...
    path.write_text("ECONOMICS WERE NOT RUN", encoding="utf-8")
    with pytest.raises(C11AResearchProgramGuardError, match="marker mismatch"):
...
def test_registry_rejects_authority_path_escape(tmp_path: Path) -> None:
    ...
    with pytest.raises(C11AResearchProgramGuardError, match="escapes"):
        validate_registry(registry, authority_root=tmp_path)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda payload: payload.update({"rank_direction": "LONG_LOW_SHORT_HIGH"}),
            "frozen field drift",
        ),
        (
            lambda payload: payload.update({"regression_lookback_returns": 168}),
            "frozen field drift",
        ),
        (
...
def test_config_rejects_signal_comparator_gate_or_safety_drift(
    mutation: object, message: str
) -> None:
    config = copy.deepcopy(_object(C11A_CONFIG_PATH))
    registry = _object(REGISTRY_PATH)
    assert callable(mutation)
    mutation(config)
    with pytest.raises(C11AResearchProgramGuardError, match=message):
...
def test_c11a_is_structurally_distinct_from_c10a() -> None:
    c11a = _object(C11A_CONFIG_PATH)
    c10a_path = C11A_CONFIG_PATH.with_name("c10a_cross_sectional_residual_momentum.json")
    c10a = _object(c10a_path)
    assert c11a["candidate_id"] != c10a["candidate_id"]
    assert c11a["score"] != c10a["score"]
    assert c11a["comparators"] != c10a["comparators"]
    assert "momentum" not in str(c11a["candidate_id"]).lower()
    assert "funding" not in str(c11a["score"]).lower()


def test_contract_is_design_only_and_does_not_claim_profit() -> None:
    contract = CONTRACT_PATH.read_text(encoding="utf-8")
    assert "Historical economic result: `NOT_RUN`" in contract
    assert "C11A is prospective economic trial `627`" in contract
    assert "C10A contributes zero economic trials" in contract
    assert "No relatively best ineligible result" in contract
    assert "actual `fundingTime`" in contract
