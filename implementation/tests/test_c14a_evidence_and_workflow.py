...
import atos.c14a_historical_evidence as evidence_module
import scripts.c14a_h1_h5_evaluate as evaluate_script
...
from atos.c14a_historical_capture import C14ACapturePackage
from atos.c14a_historical_evidence import (
    C14AEvidencePackage,
...
from scripts.c14a_h1_h5_artifact_manifest import (
    C14AArtifactManifestError,
...
def test_capture_manifest_is_recursive_bound_and_tamper_evident(tmp_path: Path) -> None:
    ...
    target.write_text("[]\n", encoding="utf-8")
    with pytest.raises(C14AHistoricalDataEvidenceError, match="hash mismatch"):
...
def test_capture_record_to_raw_chain_is_independently_recomputed(tmp_path: Path) -> None:
    ...
    with pytest.raises(C14AHistoricalDataEvidenceError, match="record/raw digest"):
        verify_capture_package(root, implementation_sha=SHA)


def test_evidence_writer_is_atomic_no_overwrite_and_no_escape(tmp_path: Path) -> None:
    package = C14AEvidencePackage(tmp_path / "evidence")
    package.write_json("final.json", {"classification": "ECONOMIC_FAIL"})
    with pytest.raises(C14AHistoricalEvidenceError, match="already exists"):
        package.write_json("final.json", {})
    with pytest.raises(C14AHistoricalEvidenceError, match="escapes"):
        package.write_json("../escape.json", {})
    manifest = package.finalize(implementation_sha=SHA)
    assert manifest["stage"] == "C14A_H1_H5_HISTORICAL_EVIDENCE_PACKAGE"
    ...
    with pytest.raises(C14AHistoricalEvidenceError, match="hash mismatch"):
        verify_evidence_package(tmp_path / "evidence", implementation_sha=SHA)
...
def test_outer_authority_artifact_seals_logs_environment_and_evidence(
    ...
    with pytest.raises(C14AArtifactManifestError, match="already sealed"):
        seal_authority_artifact(
            root,
            implementation_sha=SHA,
            authoritative_run_id="123456",
...
def test_checkout_binding_requires_exact_clean_sha(tmp_path: Path) -> None:
    ...
    with pytest.raises(C14AHistoricalRunGuardError):
        validate_checkout_binding(
...
def test_economic_fail_is_expected_but_program_failure_is_not() -> None:
    ...
    with pytest.raises(C14AHistoricalEvidenceError, match="recomputation failed"):
