...
def test_f2_full_rd(): assert [tuple(c) for c in _db().connection.execute("PRAGMA table_info(risk_decisions)").fetchall()]==RD2
def test_f2_full_ei(): assert [tuple(c) for c in _db().connection.execute("PRAGMA table_info(execution_intents)").fetchall()]==EI2
def test_f2_full_da(): assert [tuple(c) for c in _db().connection.execute("PRAGMA table_info(dispatch_attempts)").fetchall()]==DA2
...
def _fk_groups(db, table):
    groups={}
    for f in db.connection.execute("PRAGMA foreign_key_list("+table+")").fetchall(): groups.setdefault(f["id"],[]).append(f)
    return {tuple((r["seq"],r["from"],r["table"],r["to"]) for r in sorted(g,key=lambda x:x["seq"])) for g in groups.values()}

def test_f3_all_fk_mappings():
    db=_db()
    assert _fk_groups(db,"risk_decisions")=={((0,"trade_intent_id","trade_intents","trade_intent_id"),)}
    assert _fk_groups(db,"execution_intents")=={
        ((0,"trade_intent_id","trade_intents","trade_intent_id"),),
        ((0,"cycle_id","runtime_cycles","cycle_id"),),
        ((0,"risk_decision_id","risk_decisions","risk_decision_id"),(1,"trade_intent_id","risk_decisions","trade_intent_id")),
    }
    assert _fk_groups(db,"dispatch_attempts")=={((0,"execution_intent_id","execution_intents","execution_intent_id"),)}
    assert _fk_groups(db,"execution_states")=={
        ((0,"execution_intent_id","execution_intents","execution_intent_id"),),
        ((0,"execution_intent_id","dispatch_attempts","execution_intent_id"),(1,"last_attempt_id","dispatch_attempts","attempt_id")),
    }

def _idx_cols(db, name): return [r["name"] for r in db.connection.execute("PRAGMA index_info("+name+")").fetchall()]

def test_f4_idx_cols_6_of_6():
    db=_db()
    expected={
        "idx_risk_decisions_trade_intent":["trade_intent_id"],
        "idx_execution_intents_cycle":["cycle_id"],
        "idx_execution_intents_trade_intent":["trade_intent_id"],
        "idx_dispatch_attempts_execution":["execution_intent_id"],
        "idx_dispatch_attempts_client_order":["client_order_id"],
        "idx_execution_states_status":["status"],
    }
    for name, cols in expected.items(): assert _idx_cols(db,name)==cols

def _has_unique_backing(db, table, cols):
    for idx in db.connection.execute("PRAGMA index_list("+table+")").fetchall():
        if idx["unique"]==1 and _idx_cols(db,idx["name"])==cols: return True
    return False

def test_f4_unique_parent_backing_risk(): assert _has_unique_backing(_db(),"risk_decisions",["risk_decision_id","trade_intent_id"])
def test_f4_unique_parent_backing_dispatch(): assert _has_unique_backing(_db(),"dispatch_attempts",["execution_intent_id","attempt_id"])
