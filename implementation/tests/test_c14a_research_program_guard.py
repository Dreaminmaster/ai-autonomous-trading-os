...
def test_repository_history_and_c14a_design_are_frozen() -> None:
    ...
    assert report["authority_count"] == 15
...
def test_registry_records_c13a_as_one_trial_economic_fail() -> None:
    ...
    assert counts["C13A"] == 1
    assert results["C13A"] == "ECONOMIC_FAIL"
    assert sum(counts.values()) == 628
    assert registry["prospective_stage"] == "C14A"
    assert registry["familywise_trial_count"] == 629


def test_bonferroni_boundary_is_exact_and_fail_closed() -> None:
    threshold = (Decimal(1) - Decimal("0.05") / Decimal(629)).next_plus()
    ...
    with pytest.raises(C14AResearchProgramGuardError, match="trial-count drift"):
        bonferroni_adjusted_psr(threshold, trial_count=628)
    with pytest.raises(C14AResearchProgramGuardError, match=r"\[0, 1\]"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda payload: payload.update({"familywise_trial_count": 628}),
...
def test_registry_rejects_history_rewrite_or_promotion(
    ...
    mutation(registry)
    with pytest.raises(C14AResearchProgramGuardError, match=message):
...
def test_registry_hashes_all_authorities_and_rejects_tamper(tmp_path: Path) -> None:
    ...
    assert authorities[-1]["stage"] == "C13A"
    ...
    path.write_text("C13A RESULT REMOVED", encoding="utf-8")
    with pytest.raises(C14AResearchProgramGuardError, match="marker mismatch"):
...
def test_registry_rejects_authority_path_escape(tmp_path: Path) -> None:
    ...
    with pytest.raises(C14AResearchProgramGuardError, match="escapes"):
        validate_registry(registry, authority_root=tmp_path)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda payload: payload.update({"score": "INVERSE_QUOTE_VOLUME"}),
            "frozen field drift",
        ),
        (
            lambda payload: payload.update(
                {"daily_quote_volume": "SUM_24_CONFIRMED_HOURLY_VOL_CCY"}
            ),
            "frozen field drift",
        ),
        (lambda payload: payload["instruments"].pop(), "instrument drift"),
        (
...
def test_config_rejects_signal_contract_gate_or_safety_drift(
    ...
    config = copy.deepcopy(_object(C14A_CONFIG_PATH))
    registry = _object(REGISTRY_PATH)
    assert callable(mutation)
    mutation(config)
    with pytest.raises(C14AResearchProgramGuardError, match=message):
...
def test_c14a_is_not_a_renamed_lottery_volatility_or_funding_candidate() -> None:
    c14a = _object(C14A_CONFIG_PATH)
    c13a = _object(
        C14A_CONFIG_PATH.with_name("c13a_cross_sectional_lottery_demand.json")
    )
    c11a = _object(
        C14A_CONFIG_PATH.with_name("c11a_cross_sectional_idiosyncratic_volatility.json")
    )
    assert c14a["candidate_id"] != c13a["candidate_id"]
    assert c14a["score"] not in {c13a["score"], c11a["score"]}
    assert c14a["signal_lookback_complete_days"] == 28
    assert "QUOTE_VOLUME" in str(c14a["score"])
    assert "MAXIMUM" not in str(c14a["score"])
    assert "OLS" not in str(c14a["score"])
    assert "FUNDING" not in str(c14a["score"])
    assert "MeanAbsoluteReturnRankComparator" in c14a["comparators"]
    assert "InverseQuoteVolumeRankComparator" in c14a["comparators"]


def test_contract_is_design_only_and_discloses_volume_semantics() -> None:
    contract = CONTRACT_PATH.read_text(encoding="utf-8")
    assert "Historical economic result: `NOT_RUN`" in contract
    assert "C14A is prospective economic trial `629`" in contract
    assert "No C14A score, rank, trade, PnL, return, Sharpe, PSR, comparator" in contract
    assert "Liquidity risk and\nexpected cryptocurrency returns" in contract
    assert "exact 24 confirmed hourly `volCcyQuote`" in contract
