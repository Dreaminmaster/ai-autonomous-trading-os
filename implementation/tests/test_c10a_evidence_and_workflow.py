...
import atos.c10a_historical_evidence as evidence_module
import scripts.c10a_h1_h5_evaluate as evaluate_script
from atos.c7a_historical_capture import CaptureRecord
from atos.c7a_okx_public_data import build_trade_candle_request
from atos.c10a_contract import BTC_BETA_BENCHMARK, CANDIDATE_POOL, capture_plan
from atos.c10a_historical_capture import C10ACapturePackage, select_formation_universe
...
from scripts.c10a_h1_h5_artifact_manifest import (
    C10AArtifactManifestError,
...
def _binding() -> dict[str, object]:
    return {
        "schema_version": 1,
        "stage": "C10A_HISTORICAL_CHECKOUT_BINDING",
...
def _capture(tmp_path: Path) -> Path:
    ...
    package = C10ACapturePackage(root)
...
def test_capture_manifest_is_recursive_bound_and_tamper_evident(tmp_path: Path) -> None:
    ...
    target.write_text("[]\n", encoding="utf-8")
    with pytest.raises(C10AHistoricalDataEvidenceError, match="hash mismatch"):
...
def test_capture_record_to_raw_chain_is_independently_recomputed(tmp_path: Path) -> None:
    ...
    with pytest.raises(C10AHistoricalDataEvidenceError, match="record/raw digest"):
        verify_capture_package(root, implementation_sha=SHA)


def test_evidence_writer_is_atomic_no_overwrite_and_no_escape(tmp_path: Path) -> None:
    package = C10AEvidencePackage(tmp_path / "evidence")
    package.write_json("final.json", {"classification": "ECONOMIC_FAIL"})
    with pytest.raises(C10AHistoricalEvidenceError, match="already exists"):
        package.write_json("final.json", {})
    with pytest.raises(C10AHistoricalEvidenceError, match="escapes"):
        package.write_json("../escape.json", {})
    manifest = package.finalize(implementation_sha=SHA)
    assert manifest["stage"] == "C10A_H1_H5_HISTORICAL_EVIDENCE_PACKAGE"
    ...
    with pytest.raises(C10AHistoricalEvidenceError, match="hash mismatch"):
        verify_evidence_package(tmp_path / "evidence", implementation_sha=SHA)
...
def test_outer_authority_artifact_seals_logs_environment_and_evidence(
    ...
    with pytest.raises(C10AArtifactManifestError, match="already sealed"):
        seal_authority_artifact(
            root,
            implementation_sha=SHA,
            authoritative_run_id="123456",
...
def test_checkout_binding_requires_exact_clean_sha(tmp_path: Path) -> None:
    ...
    with pytest.raises(C10AHistoricalRunGuardError):
        validate_checkout_binding(
...
def test_economic_fail_is_expected_but_program_failure_is_not() -> None:
    ...
    with pytest.raises(C10AHistoricalEvidenceError, match="recomputation failed"):
        evaluate_script.classification_exit_code("PROGRAM_FAILURE")


def test_authority_workflow_is_explicit_main_only_and_one_shot_scoped() -> None:
    text = (ROOT / ".github" / "workflows" / "freqtrade-validation.yml").read_text(
        encoding="utf-8"
    )
    assert text.count("c10a_h1_h5_authoritative:") == 1
    assert text.count("c10a-h1-h5-authoritative:") == 1
    assert "github.event_name == 'workflow_dispatch'" in text
    assert "github.ref == 'refs/heads/main'" in text
    assert "inputs.c10a_h1_h5_authoritative == true" in text
    assert "scripts/c10a_h1_h5_capture.py" in text
    assert "scripts/c10a_h1_h5_evaluate.py" in text
    assert "scripts/c10a_h1_h5_artifact_manifest.py" in text
    assert "if [ \"${status}\" -gt 1 ]" in text
