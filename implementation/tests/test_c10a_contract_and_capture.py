...
from atos.c10a_historical_capture import (
    C10ACapturePackage,
...
def test_c10a_contract_binds_program_history_clocks_and_safety() -> None:
...
def test_c10a_public_request_policy_is_explicit_and_account_free() -> None:
    request = build_trade_candle_request(
        "SOL-USDT-SWAP",
        after_ms=1_704_067_200_000,
        allowed_instruments=CANDIDATE_POOL,
    )
    validate_c10a_public_request(request)
    mark = build_mark_price_request(
        "TRX-USDT-SWAP",
        after_ms=1_704_067_200_000,
        allowed_instruments=CANDIDATE_POOL,
    )
    validate_c10a_public_request(mark)
...
def test_formation_rejects_missing_misaligned_or_nonfinite_volume() -> None:
    ...
    with pytest.raises(C10AHistoricalCaptureError, match="candidate-pool"):
    ...
    with pytest.raises(C10AHistoricalCaptureError, match="misaligned"):
    ...
    with pytest.raises(C10AHistoricalCaptureError, match="finite"):
...
def test_independent_formation_recompute_requires_complete_fixed_clock() -> None:
    ...
    with pytest.raises(C10AHistoricalIndependentError, match="formation clock"):
...
def test_trade_capture_persists_raw_before_exact_normalization(tmp_path: Path) -> None:
    package = C10ACapturePackage(tmp_path / "capture")
...
def test_capture_rejects_cross_page_duplicate_before_overwrite(tmp_path: Path) -> None:
    ...
    with pytest.raises(C10AHistoricalCaptureError, match="strictly older|duplicate"):
        capture_trade_range(
            package,
            series_type="formation_trades",
            instrument="BTC-USDT-SWAP",
...
def test_package_freezes_ranked_universe_and_rejects_forgery(tmp_path: Path) -> None:
    ...
    package = C10ACapturePackage(tmp_path / "good")
    ...
    with pytest.raises(C10AHistoricalCaptureError, match="already frozen"):
        package.freeze_universe(evidence)
    with pytest.raises(C10AHistoricalCaptureError, match="outside frozen phase"):
        package.retain_c10a_series(
            series_type="marks",
    ...
    other = C10ACapturePackage(tmp_path / "forged")
    with pytest.raises(C10AHistoricalCaptureError, match="rank/selection"):
...
def test_funding_downloads_are_cross_file_unique_and_bound_to_top_eight(
    ...
    other = C10ACapturePackage(tmp_path / "duplicate")
    other.freeze_universe(evidence)
    with pytest.raises(C10AHistoricalCaptureError, match="request IDs"):
        capture_funding_downloads(
            other,
            selected_universe=selected,
            specs=duplicate,
...
def test_package_finalize_fails_until_every_frozen_series_exists(tmp_path: Path) -> None:
    ...
    with pytest.raises(C10AHistoricalCaptureError, match="inventory"):
        package.finalize(
