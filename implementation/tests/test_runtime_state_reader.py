...
# ═══════════════════════════════════════════════════════════════
# Tampered legacy DB — no CHECK constraints (P3)
# ═══════════════════════════════════════════════════════════════

def _tampered_db():
    """DB with production shape but no CHECK constraints. Allows invalid data."""
    d = tempfile.mkdtemp()
    path = Path(d) / "test.db"
...
# ═══════════════════════════════════════════════════════════════
# Cycle queries — scoped + cross-session + tie-break (P5)
# ═══════════════════════════════════════════════════════════════
...
def test_list_incomplete_scoped_cross_session():
    db = _tampered_db()
    ...
    assert 'c1' in ids; assert 'c2' not in ids; assert 'c3' not in ids
    assert len(cycles) == 1
...
def test_cycle_tie_break():
    ...
    r = RuntimeStateReader(db)
    cycles = r.list_incomplete_cycles("s1")
    assert cycles[0].cycle_id == 'cA'
    assert cycles[1].cycle_id == 'cB'

# ═══════════════════════════════════════════════════════════════
# Recovery queries — scoped + cross-session (P5)
# ═══════════════════════════════════════════════════════════════
...
def test_recovery_cross_session():
    ...
    assert 'r1' in ids; assert 'r2' in ids; assert 'r3' not in ids; assert 'r4' not in ids
    assert len(recs) == 2
...
def test_recovery_tie_break():
    ...
    r = RuntimeStateReader(db)
    recs = r.list_unresolved_recoveries("s1")
    assert recs[0].recovery_id == 'rA'
    assert recs[1].recovery_id == 'rX'

# ═══════════════════════════════════════════════════════════════
# Production-path corruption — all 9 via real Reader (P4)
# ═══════════════════════════════════════════════════════════════
...
def test_corrupt_non_string():
    db = _tampered_db()
    db.connection.execute("INSERT INTO runtime_sessions VALUES ('s1','t','paper','RUNNING',NULL,NULL)")
    db.connection.execute("INSERT INTO recovery_states VALUES ('r1','s1','PENDING',42,'t',NULL)")
    r = RuntimeStateReader(db)
    with pytest.raises(StateDataCorruptionError): r.get_recovery("r1")
