...
def _signal_marks(
    ...
    start = decision - (REGRESSION_LOOKBACK_RETURNS + 2) * HOUR
    end = decision
    times = []
    current = start
    while current < end:
    ...
    for asset_index, instrument in enumerate(SELECTED):
        ...
        for index, stamp in enumerate(times):
            ...
            idiosyncratic = (asset_index + 1) * 0.000012 * math.sin(
                index / (11.0 + asset_index)
            )
            price *= math.exp(common + idiosyncratic)
...
def _window_sources():
    ...
    mark_start = window.start - (REGRESSION_LOOKBACK_RETURNS + 2) * HOUR
    mark_times = []
    current = mark_start
...
def test_signal_uses_exact_672_returns_and_excludes_t_minus_one_candle() -> None:
    ...
    assert first["regression_return_count"] == 672
    assert len(first["longs"]) == 2
    assert len(first["shorts"]) == 2
    assert first["longs"] == ["LINK-USDT-SWAP", "DOT-USDT-SWAP"]
    assert first["shorts"] == ["AVAX-USDT-SWAP", "ADA-USDT-SWAP"]
...
def test_signal_rejects_missing_hour_or_zero_factor_variance() -> None:
    ...
    with pytest.raises(C11AHistoricalReplayError, match="missing exact mark hour"):
    ...
    with pytest.raises(C11AHistoricalReplayError, match="factor variance"):
        build_signal(decision, selected_universe=SELECTED, mark_closes=constant)


def test_total_volatility_comparator_has_same_fixed_long_short_geometry() -> None:
    decision = datetime(2024, 1, 1, tzinfo=UTC)
    signal = build_signal(
        decision,
        selected_universe=SELECTED,
        mark_closes=_signal_marks(decision),
        policy="TotalVolatilityComparator",
    )
    assert signal["policy"] == "TotalVolatilityComparator"
    assert signal["longs"] == ["ETH-USDT-SWAP", "LINK-USDT-SWAP"]
    assert signal["shorts"] == ["AVAX-USDT-SWAP", "ADA-USDT-SWAP"]
    assert len(signal["longs"]) == 2
    assert len(signal["shorts"]) == 2
    assert set(signal["longs"]).isdisjoint(signal["shorts"])
    assert all(
        float(row["total_volatility_score"]) > 0
        and float(row["idiosyncratic_volatility_score"]) > 0
...
def test_window_rejects_missing_trade_hour_and_unaccounted_source_shape() -> None:
    ...
    with pytest.raises(C11AHistoricalReplayError, match="misaligned|missing exact"):
        evaluate_historical_window(
            "H1",
            selected_universe=SELECTED,
            trade_rows=trade_rows,
            ...
            for cost_index, cost_label in enumerate(("1.0x", "1.5x", "2.0x")):
                if policy == "CashComparator":
                    ...
                    base = {
                        ...
                        "TotalVolatilityComparator": Decimal("0.010"),
                    ...
                    weekly_level, weekly_noise = {
                        ...
                        "TotalVolatilityComparator": (Decimal("0.0005"), 0.0015),
                        "AlwaysLongSelectedUniverseComparator": (
                            Decimal("0.001"),
                            0.001,
                        ),
...
def test_pooled_summary_applies_program_correction_and_total_volatility_gates() -> None:
...
def test_independent_window_recompute_rejects_event_time_or_type_tamper() -> None:
    trade_rows, mark_rows, funding_rows = _window_sources()
    producer = evaluate_historical_window_matrix(
        "H1",
        selected_universe=SELECTED,
    ...
    replay = producer["replays"][CANDIDATE_ID]["1.0x"]
    replay["complete_hourly_equity_path"][1]["timestamp"] = (
        "2024-01-01T00:00:01Z"
    )
    replay["complete_hourly_equity_path"][2]["event"] = "TAMPERED"
    ...
    checks = review["replay_reviews"][f"{CANDIDATE_ID}:1.0x"]["checks"]
    assert review["status"] == "FAIL"
    assert checks["complete_path_recompute"] is False
...
def test_independent_module_does_not_import_production_engine() -> None:
    source = (
        ...
        / "src"
        / "atos"
        / "c11a_historical_independent.py"
    ).read_text(encoding="utf-8")
    assert "from atos.c11a_historical_replay" not in source
    assert "from atos.c11a_historical_signal" not in source
    assert "from atos.c11a_historical_ledger" not in source
