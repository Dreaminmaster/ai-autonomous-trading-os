"""Seal the complete outer C13A authority directory with SHA-256 custody."""
...
from atos.c13a_contract import safety_boundary

EXACT_SHA = re.compile(r"[0-9a-f]{40}")
SAFE_RUN_ID = re.compile(r"[A-Za-z0-9][A-Za-z0-9._-]{0,159}")


class C13AArtifactManifestError(RuntimeError):
...
def seal_authority_artifact(
    ...
    if not EXACT_SHA.fullmatch(implementation_sha) or not SAFE_RUN_ID.fullmatch(
        ...
        raise C13AArtifactManifestError("implementation SHA or run ID is invalid")
    authority = Path(root)
    if authority.is_symlink():
        raise C13AArtifactManifestError("authority artifact root is a symbolic link")
    ...
    if destination.exists() or temporary.exists():
        raise C13AArtifactManifestError("authority artifact is already sealed")
    entries = sorted(authority.rglob("*"))
    if any(path.is_symlink() for path in entries):
        raise C13AArtifactManifestError("authority artifact contains a symbolic link")
    files = []
    ...
    manifest = {
        "schema_version": 1,
        "stage": "C13A_H1_H5_OUTER_AUTHORITY_ARTIFACT",
    ...
    try:
        ...
        raise C13AArtifactManifestError(
