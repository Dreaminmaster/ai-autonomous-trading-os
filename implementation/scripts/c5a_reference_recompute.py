"""Independent C5A metrics, gates, and full recomputation."""
...
from .c5a_reference_policy import signal_snapshot, solve_post_cost
from .c5a_reference_simulation import _max_drawdown, simulate_policy_half
...
def _weekly_psr(values: Sequence[float]) -> dict[str, float]:
    ...
    if len(array) != 26 or not np.isfinite(array).all():
        raise C5AReferenceError("C5A PSR requires exactly 26 finite weekly returns")
    mean_value = float(np.mean(array))
    deviation = float(np.std(array, ddof=1))
    if deviation == 0:
        if mean_value != 0:
            raise C5AReferenceError("nonzero weekly mean with zero variance")
        return {
            "weekly_mean": 0.0,
            "weekly_std": 0.0,
            "sr_weekly_raw": 0.0,
    ...
    if not math.isfinite(radicand) or radicand <= 0:
        raise C5AReferenceError("invalid C5A PSR radicand")
...
def aggregate_policy(
    ...
    *,
    ...
    if len(ordered) != 2:
        raise C5AReferenceError("C5A aggregate requires two half windows")
    ...
    if abs(sum(contributions.values()) - sum(half_pnl)) > 1e-9:
        raise C5AReferenceError("aggregate C5A contribution reconciliation failure")
    return result
...
def simulate_comparator_half(
    ...
    *,
    ...
    if comparator_id not in COMPARATORS:
        raise C5AReferenceError("unknown C5A comparator")
    ...
    if len(indices) != 546:
        raise C5AReferenceError("comparator half-window count mismatch")
    ...
    instruments = (("BTC-USDT",) if comparator_id == "btc_buy_hold" else SPOT_INSTRUMENTS)
    ...
    for local_index, index in enumerate(indices):
        ...
        if local_index == len(indices) - 1:
            ...
            out = solve_post_cost(equity, current, {}, float(config["cost_rates"][cost_label]))
    ...
    if len(weekly_returns) != 13:
        raise C5AReferenceError("comparator weekly count mismatch")
    return {
        "comparator_id": comparator_id,
        "window_id": str(window["id"]),
        "cost_label": cost_label,
...
def decide(
    ...
    by_key = {(str(row["policy_id"]), str(row["cost_label"])): row for row in aggregates}
...
def run_screen(
    ...
    if len(policy_rows) != 12 or len(comparator_rows) != 18:
        raise C5AReferenceError("C5A frozen cell count mismatch")
    aggregates = [
        aggregate_policy(policy_rows, policy_id=policy, cost_label=cost, config=config)
        for policy in (CANDIDATE_ID, ABLATION_ID)
        for cost in COST_LABELS
    ]
...
reference_prepare_market = prepare_market
reference_build_calibration = build_calibration
reference_signal_snapshot = signal_snapshot
reference_solve_post_cost = solve_post_cost
reference_simulate_policy_half = simulate_policy_half
reference_aggregate_policy = aggregate_policy
reference_run_screen = run_screen
