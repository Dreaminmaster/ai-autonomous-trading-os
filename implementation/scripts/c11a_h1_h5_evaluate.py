"""One-shot C11A H1-H5 evaluation over verified official-public custody."""
...
from atos.c11a_historical_independent import C11AHistoricalIndependentError
from atos.c11a_historical_replay import C11AHistoricalReplayError
...
def classification_exit_code(classification: str) -> int:
    ...
    raise C11AHistoricalEvidenceError(
        "C11A independent recomputation failed after evidence retention"
    )
...
def cli() -> int:
    try:
        return main()
    except C11AHistoricalDataEvidenceError as exc:
        print(
            json.dumps(
                {
                    "status": "FAIL",
