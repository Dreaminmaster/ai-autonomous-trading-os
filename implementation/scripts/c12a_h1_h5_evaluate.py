"""One-shot C12A evaluation over verified official-public custody."""
...
from atos.c12a_historical_independent import C12AHistoricalIndependentError
from atos.c12a_historical_replay import C12AHistoricalReplayError
...
def classification_exit_code(classification: str) -> int:
    ...
    raise C12AHistoricalEvidenceError(
        "C12A independent recomputation failed after evidence retention"
    )
...
def cli() -> int:
    try:
        return main()
    except C12AHistoricalDataEvidenceError as exc:
        print(
            json.dumps(
                {
                    "status": "FAIL",
