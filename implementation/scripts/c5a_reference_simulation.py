"""Independent C5A half-window accounting recomputation."""
...
from .c5a_reference_policy import (
    _weights,
    one_way_distance,
    signal_snapshot,
    solve_post_cost,
...
def _max_drawdown(equity: Sequence[float]) -> float:
    ...
    for raw in equity:
        ...
        if value <= 0:
            raise C5AReferenceError("equity must remain positive")
        peak = max(peak, value)
...
def simulate_policy_half(
    ...
    *,
    ...
    if cost_label not in COST_LABELS:
        raise C5AReferenceError("unknown C5A cost label")
    ...
    if len(indices) != 546:
        raise C5AReferenceError(f"C5A half-window bar count mismatch: {len(indices)}")
    decisions = {
    ...
    for local_index, index in enumerate(indices):
        ...
        if index in decisions:
            if week_start_equity is not None:
                raise C5AReferenceError("prior weekly bucket did not close")
        ...
        if final_bar and any(quantity > 0 for quantity in quantities.values()):
            ...
            terminal_distance = one_way_distance(asset_weights, cash_weight, {}, 1.0)
            current_values = {
        ...
        if not math.isfinite(close_equity) or close_equity <= 0:
            raise C5AReferenceError("invalid C5A close equity")
        ...
        if timestamp.weekday() == 6 and timestamp.hour == 20:
            if week_start_equity is None or week_start_time is None:
                raise C5AReferenceError("missing weekly start reference")
            weekly.append(
                {
                    "window_id": str(window["id"]),
    ...
    if week_start_equity is not None or week_start_time is not None:
        raise C5AReferenceError("unfinished weekly bucket")
    if len(weekly) != 13 or len(decision_rows) != 13:
        raise C5AReferenceError("C5A half evidence count mismatch")
    if any(quantity != 0.0 for quantity in quantities.values()):
        raise C5AReferenceError("C5A half did not finish in cash")
    final_equity = equity_curve[-1]
    if abs(sum(contributions.values()) - (final_equity - float(config["starting_equity"]))) > 1e-9:
        raise C5AReferenceError("C5A asset contribution reconciliation failure")
    if abs(sum(item["net_pnl"] for item in weekly) - (final_equity - float(config["starting_equity"]))) > 1e-9:
        raise C5AReferenceError("C5A weekly PnL reconciliation failure")
    return {
        "schema_version": 1,
        "stage": "C5A",
        "status": "PASS",
