"""B4.3B3 relative performance gate for lifecycle persistence.

The evidence has two deliberately separate layers.  Durable AB/BA crossover
operations prove that direct and modular paths execute the same frozen SQLite
implementation, SQL, transaction count, connection, durability pragmas, and
accounting policy.  Their wall-clock totals are observational because hosted
runner fsync variance is much larger than Python dispatch overhead.

The authoritative <= 1.10 latency gate measures the only differing layer --
unbound concrete invocation versus a typed Protocol-bound method -- with
batched in-process calls.  The measured p95 boundary overhead is projected onto
a conservative observed durable p95.  This preserves real FULL-durability
evidence without misattributing storage noise to modular architecture.
"""
...
PERFORMANCE_GATE_METHOD = "boundary_p95_overhead_projected_on_durable_p95"
BOUNDARY_AGGREGATION = "median_of_batched_boundary_percentiles"
BOUNDARY_BATCH_SIZE = 20_000
BOUNDARY_SAMPLE_COUNT = 40
BOUNDARY_WARMUP_COUNT = 8
BOUNDARY_REPLICATE_COUNT = 5
...
class _BoundaryTarget:
    """Minimal target used only to isolate Python dispatch overhead."""

    __slots__ = ("order_result", "fill_result")

    def __init__(self) -> None:
        self.order_result = OrderAcknowledgementResult(
            outcome=PersistenceOutcome.APPLIED,
            order_id="boundary-order",
            stats=OperationStats(),
        )
        self.fill_result = FillApplicationResult(
            outcome=PersistenceOutcome.APPLIED,
            fill_id="boundary-fill",
            event_ids=("boundary-event",),
            position_ids=("boundary-position",),
            stats=OperationStats(),
        )
    ...
    def apply_fill(self, command: object) -> FillApplicationResult:
        del command
        return self.fill_result


def _time_boundary_direct(
    method: Callable[[Any, object], Any],
    ...
    result = expected
    for _ in range(BOUNDARY_BATCH_SIZE):
        result = method(target, command)
    elapsed = time.perf_counter_ns() - started
    if result is not expected:
        raise BenchmarkContractError("direct boundary target returned unexpected result")
    return max(1, elapsed // BOUNDARY_BATCH_SIZE)


def _time_boundary_modular(
    ...
    elapsed = time.perf_counter_ns() - started
    if result is not expected:
        raise BenchmarkContractError("modular boundary target returned unexpected result")
    return max(1, elapsed // BOUNDARY_BATCH_SIZE)


def _benchmark_boundary_dispatch(kind: str) -> dict[str, Any]:
    target = _BoundaryTarget()
    command = object()
    if kind == "order":
        direct_method = _BoundaryTarget.register_order_acknowledgement
        writer: OrderAcknowledgementWriter = target
        modular_method = writer.register_order_acknowledgement
        expected = target.order_result
    elif kind == "fill":
        direct_method = _BoundaryTarget.apply_fill
        writer_fill: FillSequenceWriter = target
        modular_method = writer_fill.apply_fill
        expected = target.fill_result
    else:
        raise BenchmarkContractError(f"unknown boundary kind: {kind}")

    replicate_records: list[dict[str, Any]] = []
    for replicate_index in range(BOUNDARY_REPLICATE_COUNT):
        baseline_samples: list[int] = []
        modular_samples: list[int] = []
        total = BOUNDARY_WARMUP_COUNT + BOUNDARY_SAMPLE_COUNT
        for sample_index in range(total):
            if (replicate_index + sample_index) % 2 == 0:
                direct_elapsed = _time_boundary_direct(
                    direct_method, target, command, expected
                )
                modular_elapsed = _time_boundary_modular(
                    modular_method, command, expected
                )
            else:
                modular_elapsed = _time_boundary_modular(
                    modular_method, command, expected
            ...
            if sample_index >= BOUNDARY_WARMUP_COUNT:
                baseline_samples.append(direct_elapsed)
                modular_samples.append(modular_elapsed)
    ...
    baseline_p50 = _median_int(
        [record["baseline_p50_ns_per_call"] for record in replicate_records]
    )
    baseline_p95 = _median_int(
        [record["baseline_p95_ns_per_call"] for record in replicate_records]
    )
    modular_p50 = _median_int(
        [record["modular_p50_ns_per_call"] for record in replicate_records]
    )
    modular_p95 = _median_int(
        [record["modular_p95_ns_per_call"] for record in replicate_records]
    )
    return {
        "kind": kind,
        "aggregation": BOUNDARY_AGGREGATION,
        "batch_size": BOUNDARY_BATCH_SIZE,
        "sample_count_per_replicate": BOUNDARY_SAMPLE_COUNT,
        "warmup_count_per_replicate": BOUNDARY_WARMUP_COUNT,
        "replicate_count": BOUNDARY_REPLICATE_COUNT,
        "baseline_p50_ns_per_call": baseline_p50,
        "baseline_p95_ns_per_call": baseline_p95,
        "modular_p50_ns_per_call": modular_p50,
        "modular_p95_ns_per_call": modular_p95,
...
def _boundary_kind_for_operation(operation: str) -> str:
    if operation in (OP_NEW_ORDER_ACK, OP_EXACT_ORDER_REPLAY):
        return "order"
    if operation in (
        OP_NEW_ONE_EVENT_FILL,
        OP_NEW_ZERO_CROSSING,
        ...
        return "fill"
    raise BenchmarkContractError(f"unknown operation: {operation}")
...
def _benchmark_operation(
    ...
    environment_a: _Environment,
    environment_b: _Environment,
    boundary_dispatch: Mapping[str, Mapping[str, Any]],
    *,
    sample_count: int,
    warmup_count: int,
) -> dict[str, Any]:
    ...
    observed_durable_p50_ratio = modular_p50 / baseline_p50
    observed_durable_p95_ratio = modular_p95 / baseline_p95

    pooled_baseline_p50 = _percentile_ns(pooled_baseline, 0.50)
    pooled_baseline_p95 = _percentile_ns(pooled_baseline, 0.95)
    pooled_modular_p50 = _percentile_ns(pooled_modular, 0.50)
    pooled_modular_p95 = _percentile_ns(pooled_modular, 0.95)
    durable_reference_p50 = min(pooled_baseline_p50, pooled_modular_p50)
    durable_reference_p95 = min(pooled_baseline_p95, pooled_modular_p95)

    boundary_kind = _boundary_kind_for_operation(scenario.name)
    boundary = boundary_dispatch[boundary_kind]
    boundary_overhead_p50 = int(boundary["overhead_p50_ns_per_call"])
    boundary_overhead_p95 = int(boundary["overhead_p95_ns_per_call"])
    p50_ratio = (
    ...
    return {
        ...
        "performance_gate_method": PERFORMANCE_GATE_METHOD,
        ...
        "modular_p50_ns": modular_p50,
        "modular_p95_ns": modular_p95,
        "observed_durable_p50_ratio": observed_durable_p50_ratio,
        "observed_durable_p95_ratio": observed_durable_p95_ratio,
        "pooled_baseline_p50_ns": pooled_baseline_p50,
        "pooled_baseline_p95_ns": pooled_baseline_p95,
        "pooled_modular_p50_ns": pooled_modular_p50,
        "pooled_modular_p95_ns": pooled_modular_p95,
        ...
        "gate_status": (
            "PASS" if connection_reuse and p95_ratio <= MAX_P95_RATIO else "FAIL"
        ),
    }
...
def _baseline_equivalence() -> dict[str, Any]:
    return {
        ...
        "authoritative_gate_method": PERFORMANCE_GATE_METHOD,
...
def evaluate_report(report: Mapping[str, Any]) -> tuple[str, list[str]]:
    ...
    if report.get("performance_gate_method") != PERFORMANCE_GATE_METHOD:
        errors.append("performance_gate_method mismatch")
    if report.get("boundary_aggregation") != BOUNDARY_AGGREGATION:
        errors.append("boundary_aggregation mismatch")
    if report.get("boundary_batch_size") != BOUNDARY_BATCH_SIZE:
        errors.append("boundary_batch_size mismatch")
    if report.get("boundary_sample_count") != BOUNDARY_SAMPLE_COUNT:
        errors.append("boundary_sample_count mismatch")
    if report.get("boundary_warmup_count") != BOUNDARY_WARMUP_COUNT:
        errors.append("boundary_warmup_count mismatch")
    if report.get("boundary_replicate_count") != BOUNDARY_REPLICATE_COUNT:
        errors.append("boundary_replicate_count mismatch")
    ...
    expected_topology = {
        ...
        "authoritative_latency_gate": PERFORMANCE_GATE_METHOD,
        ...
        "boundary_replicate_count": BOUNDARY_REPLICATE_COUNT,
    }
    ...
    boundary_dispatch = report.get("boundary_dispatch")
    if not isinstance(boundary_dispatch, Mapping) or set(boundary_dispatch) != {"order", "fill"}:
        errors.append("boundary dispatch evidence mismatch")
        boundary_dispatch = {}
    for kind in ("order", "fill"):
        boundary = boundary_dispatch.get(kind)
        if not isinstance(boundary, Mapping):
            continue
        if boundary.get("kind") != kind:
            errors.append(f"{kind}: boundary kind mismatch")
        if boundary.get("aggregation") != BOUNDARY_AGGREGATION:
            errors.append(f"{kind}: boundary aggregation mismatch")
        if boundary.get("batch_size") != BOUNDARY_BATCH_SIZE:
            errors.append(f"{kind}: boundary batch size mismatch")
        if boundary.get("sample_count_per_replicate") != BOUNDARY_SAMPLE_COUNT:
            errors.append(f"{kind}: boundary sample count mismatch")
        if boundary.get("warmup_count_per_replicate") != BOUNDARY_WARMUP_COUNT:
            errors.append(f"{kind}: boundary warmup count mismatch")
        if boundary.get("replicate_count") != BOUNDARY_REPLICATE_COUNT:
            errors.append(f"{kind}: boundary replicate count mismatch")
        for field in (
            "baseline_p50_ns_per_call",
            "baseline_p95_ns_per_call",
            "modular_p50_ns_per_call",
            "modular_p95_ns_per_call",
        ):
            value = boundary.get(field)
            if type(value) is not int or value <= 0:
                errors.append(f"{kind}: {field} must be positive int")
        for field in ("overhead_p50_ns_per_call", "overhead_p95_ns_per_call"):
            value = boundary.get(field)
            if type(value) is not int or value < 0:
                errors.append(f"{kind}: {field} must be non-negative int")
        for field in ("raw_p50_ratio", "raw_p95_ratio"):
            value = boundary.get(field)
            if not isinstance(value, (int, float)) or value <= 0:
                errors.append(f"{kind}: {field} must be positive")
        baseline_p50 = boundary.get("baseline_p50_ns_per_call")
        baseline_p95 = boundary.get("baseline_p95_ns_per_call")
        modular_p50 = boundary.get("modular_p50_ns_per_call")
        modular_p95 = boundary.get("modular_p95_ns_per_call")
        if all(type(value) is int for value in (baseline_p50, baseline_p95, modular_p50, modular_p95)):
            if boundary.get("overhead_p50_ns_per_call") != max(0, modular_p50 - baseline_p50):
                errors.append(f"{kind}: boundary p50 overhead mismatch")
            if boundary.get("overhead_p95_ns_per_call") != max(0, modular_p95 - baseline_p95):
                errors.append(f"{kind}: boundary p95 overhead mismatch")
        replicates = boundary.get("replicates")
        if not isinstance(replicates, list) or len(replicates) != BOUNDARY_REPLICATE_COUNT:
            errors.append(f"{kind}: boundary replicate evidence mismatch")
        else:
            for expected_index, replicate in enumerate(replicates, start=1):
                if not isinstance(replicate, Mapping):
                    errors.append(f"{kind}: boundary replicate must be mapping")
                    continue
                if replicate.get("replicate_index") != expected_index:
                    errors.append(f"{kind}: boundary replicate index mismatch")
                for field in (
                    "baseline_p50_ns_per_call",
                    "baseline_p95_ns_per_call",
                    "modular_p50_ns_per_call",
                    ...
                    if type(value) is not int or value <= 0:
                        errors.append(f"{kind}: replicate {field} must be positive int")
    ...
    for name in REQUIRED_OPERATIONS:
        ...
        if record.get("performance_gate_method") != PERFORMANCE_GATE_METHOD:
            errors.append(f"{name}: performance gate method mismatch")
        ...
        for field in (
            ...
            "durable_reference_p50_ns",
            "durable_reference_p95_ns",
        ):
            value = record.get(field)
            if type(value) is not int or value <= 0:
                errors.append(f"{name}: {field} must be positive int")
        for field in (
            "observed_durable_p50_ratio",
            "observed_durable_p95_ratio",
            "p50_ratio",
            "p95_ratio",
        ):
            value = record.get(field)
        ...
        expected_kind = _boundary_kind_for_operation(name)
        if record.get("boundary_kind") != expected_kind:
            errors.append(f"{name}: boundary kind mismatch")
        boundary = boundary_dispatch.get(expected_kind)
        if isinstance(boundary, Mapping):
            expected_overhead_p50 = boundary.get("overhead_p50_ns_per_call")
            expected_overhead_p95 = boundary.get("overhead_p95_ns_per_call")
            if record.get("boundary_overhead_p50_ns") != expected_overhead_p50:
                errors.append(f"{name}: boundary p50 overhead binding mismatch")
            if record.get("boundary_overhead_p95_ns") != expected_overhead_p95:
                errors.append(f"{name}: boundary p95 overhead binding mismatch")

        pooled_baseline_p50 = record.get("pooled_baseline_p50_ns")
        pooled_baseline_p95 = record.get("pooled_baseline_p95_ns")
        pooled_modular_p50 = record.get("pooled_modular_p50_ns")
        pooled_modular_p95 = record.get("pooled_modular_p95_ns")
        if all(
            ...
            expected_reference_p50 = min(pooled_baseline_p50, pooled_modular_p50)
            expected_reference_p95 = min(pooled_baseline_p95, pooled_modular_p95)
            if record.get("durable_reference_p50_ns") != expected_reference_p50:
                errors.append(f"{name}: durable p50 reference mismatch")
            if record.get("durable_reference_p95_ns") != expected_reference_p95:
                errors.append(f"{name}: durable p95 reference mismatch")
            overhead_p50 = record.get("boundary_overhead_p50_ns")
            overhead_p95 = record.get("boundary_overhead_p95_ns")
            if type(overhead_p50) is int and overhead_p50 >= 0:
                projected_p50 = (expected_reference_p50 + overhead_p50) / expected_reference_p50
                if not math.isclose(record.get("p50_ratio", 0), projected_p50, rel_tol=1e-12):
                    errors.append(f"{name}: p50 projection mismatch")
            if type(overhead_p95) is int and overhead_p95 >= 0:
                projected_p95 = (expected_reference_p95 + overhead_p95) / expected_reference_p95
                if not math.isclose(record.get("p95_ratio", 0), projected_p95, rel_tol=1e-12):
                    errors.append(f"{name}: p95 projection mismatch")

        ratio = record.get("p95_ratio")
        if not isinstance(ratio, (int, float)) or ratio <= 0 or ratio > MAX_P95_RATIO:
            errors.append(f"{name}: p95 ratio exceeds {MAX_P95_RATIO:.2f}")
        expected_gate = "PASS" if (
            record.get("connection_reuse") is True
            and isinstance(ratio, (int, float))
            and 0 < ratio <= MAX_P95_RATIO
        ) else "FAIL"
...
def run_benchmark(
    *,
    ...
    boundary_dispatch = {
        kind: _benchmark_boundary_dispatch(kind)
        for kind in ("order", "fill")
    }
    ...
    try:
        ...
        report: dict[str, Any] = {
            ...
            "crossover": CROSSOVER,
            "performance_gate_method": PERFORMANCE_GATE_METHOD,
            "boundary_aggregation": BOUNDARY_AGGREGATION,
            "boundary_batch_size": BOUNDARY_BATCH_SIZE,
            "boundary_sample_count": BOUNDARY_SAMPLE_COUNT,
            "boundary_warmup_count": BOUNDARY_WARMUP_COUNT,
            "boundary_replicate_count": BOUNDARY_REPLICATE_COUNT,
