#!/usr/bin/env python3
"""Capture and verify every effective C2A source file and retained snapshot."""
...
class C2ASourceInventoryError(RuntimeError):
    pass
...
def sha256(path: Path) -> str:
    try:
        ...
        raise C2ASourceInventoryError(f"unable to hash {path}: {exc}") from exc
...
def exact_source_sha() -> str:
    value = os.environ.get("C2A_SOURCE_SHA", "")
    if not SHA_RE.fullmatch(value):
        raise C2ASourceInventoryError("C2A_SOURCE_SHA must be an exact lowercase SHA")
    return value


def validate_source_paths() -> None:
    if len(SOURCE_PATHS) != len(set(SOURCE_PATHS)):
        raise C2ASourceInventoryError("duplicate C2A effective source path")
    for path in SOURCE_PATHS:
        if path.is_absolute() or ".." in path.parts:
            raise C2ASourceInventoryError(f"unsafe C2A source path: {path}")
        if not (ROOT / path).is_file():
            raise C2ASourceInventoryError(f"missing C2A effective source: {path}")
...
def capture() -> dict[str, Any]:
    ...
    for relative in SOURCE_PATHS:
        ...
        if source_hash != retained_hash:
            raise C2ASourceInventoryError(f"snapshot hash mismatch for {relative}")
        entries.append(
            {
                "path": str(relative),
                "sha256": source_hash,
...
def verify() -> dict[str, Any]:
    ...
    try:
        ...
        raise C2ASourceInventoryError(f"invalid C2A source inventory: {exc}") from exc
    if not isinstance(payload, Mapping):
        raise C2ASourceInventoryError("C2A source inventory must be an object")
    if payload.get("status") != "PASS" or payload.get("source_head_sha") != source_sha:
        raise C2ASourceInventoryError("C2A source inventory status or SHA mismatch")
    ...
    if payload.get("paths") != expected_paths or payload.get("path_count") != len(expected_paths):
        raise C2ASourceInventoryError("C2A source inventory path drift")
    entries = payload.get("entries")
    if not isinstance(entries, list) or len(entries) != len(SOURCE_PATHS):
        raise C2ASourceInventoryError("C2A source inventory entry count mismatch")
    for relative, entry in zip(SOURCE_PATHS, entries, strict=True):
        if not isinstance(entry, Mapping) or entry.get("path") != str(relative):
            raise C2ASourceInventoryError("C2A source inventory ordering mismatch")
        source = ROOT / relative
        retained = RESULTS / str(entry.get("snapshot_path", ""))
        if not retained.is_file():
            raise C2ASourceInventoryError(f"missing retained C2A source: {relative}")
        current_hash = sha256(source)
        retained_hash = sha256(retained)
        if entry.get("sha256") != current_hash:
            raise C2ASourceInventoryError(f"current source hash mismatch: {relative}")
        if entry.get("snapshot_sha256") != retained_hash or current_hash != retained_hash:
            raise C2ASourceInventoryError(f"retained source hash mismatch: {relative}")
    if payload.get("confirmation_opened") is not False:
        raise C2ASourceInventoryError("C2A confirmation state drift")
    if payload.get("holdout_state") != "HOLDOUT_CLOSED" or payload.get("live") != "FORBIDDEN":
        raise C2ASourceInventoryError("C2A source inventory safety drift")
    return dict(payload)
...
def main() -> int:
    ...
    print(
        f"C2A source inventory {args.mode} PASS: {payload['path_count']} paths / "
