"""One-shot C14A H1-H5 evaluation over verified official-public custody."""
...
from atos.c14a_historical_independent import C14AHistoricalIndependentError
from atos.c14a_historical_replay import C14AHistoricalReplayError
...
def classification_exit_code(classification: str) -> int:
    ...
    raise C14AHistoricalEvidenceError(
        "C14A independent recomputation failed after evidence retention"
    )
...
def cli() -> int:
    try:
        return main()
    except C14AHistoricalDataEvidenceError as exc:
        print(
            json.dumps(
                {
                    "status": "FAIL",
